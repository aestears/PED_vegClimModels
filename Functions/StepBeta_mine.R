StepBeta_mine <- function (object, k = 2, dispersion = T, data) 
{
      ...
      while (g < length(dispersion_terms) + 1) {
        ...
        while (diff > 5e-15) {
          ...
          while (i < length(Terms_NoInt) + 1) {
            ...
            mod_updated <- try(betareg(new_formula, data = data, 
          ...
          while (diff > 5e-15) {
            ...
            while (i < length(Terms_Int) + 1) {
              ...
              mod_updated <- try(betareg(new_formula, 
                                         data = data, weights = beta_control_weights, 
...
# {
#   if (!identical(grep("\\:", as.character(the_formula[3])), 
#                  integer(0)) | !identical(grep("\\*", as.character(the_formula[3])), 
#                                           integer(0))) {
#     the_formula <- as.formula(paste(the_formula[2], the_formula[1], 
#                                     gsub("\\|.*", "", as.character(the_formula[3]))))
#     var_position <- grep(":", attr(terms(the_formula), "term.labels"))
#     update(the_formula, drop.terms(terms(the_formula), var_position, 
#                                    keep.response = TRUE))
#   }
#   else {
#     return(the_formula)
