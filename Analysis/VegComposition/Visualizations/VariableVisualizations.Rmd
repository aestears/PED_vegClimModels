...
```{r, echo = FALSE}
kable(modDat %>% 
        select(swe_meanAnnAvg_CLIM:durationFrostFreeDays_meanAnnAvg_CLIM) %>% 
        names()
)
```
...
(corrPlot_CLIMMean <- 
  ...
  rename("tmean" = tmean_meanAnnAvg_CLIM,
         "vp" = vp_meanAnnAvg_CLIM, "prcp" = prcp_meanAnnTotal_CLIM, 
...
# get data 
 plotDat <- 
   ...
   #st_drop_geometry() %>% 
  select(c(swe_meanAnnAvg_CLIM, tmean_meanAnnAvg_CLIM,                     
           vp_meanAnnAvg_CLIM,prcp_meanAnnTotal_CLIM, precip_driestMonth_meanAnnAvg_CLIM,        
