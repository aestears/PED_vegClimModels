...
# read in veg data --------------------------------------------------------
vegDat_temp <- readRDS("./Data_processed/CoverData/TotalTreeOnly_FIAandAIM_DataForModels_withEcoregion_sampledLANDFIRE.rds")
...
# remove unnecessary soils variables 
  ...
  # terra::writeRaster(LCMAP_use, file = "./Data_raw/LCMAP/LCMAP_reclassifiedToUse.tif")

  LCMAP_use <- terra::rast("./Data_raw/LCMAP/LCMAP_reclassifiedToUse.tif") #%>% 
...
# these are locations that were burned previously so have NA for tree cover, and where the % of tree types should have also be turned into NA
  ...
  newFinalDat_new <- newFinalDat_new %>% 
    mutate(across(c(TotalTreeCover_n:TotalTreeCover_sd), ~na_if(., NaN)))
  # save data  --------------------------------------------------------------
  
  ## save the PRE-spatially averaged data after LCMAP filtering
  saveRDS(dat_preAvg_new, 
          "./Data_processed/CoverData/TotalTreeOnly_FIAandAIM_data_beforeSpatialAveraging_sampledLANDFIRE.rds")
  
  ## save the spatially averaged data w/ soil added and after LCMAP filtering
  saveRDS(newFinalDat_new %>% st_drop_geometry(), 
          "./Data_processed/CoverData/TotalTreeOnly_FIAandAIM_DataForModels_spatiallyAveraged_withSoils_noSf_sampledLANDFIRE.rds")
