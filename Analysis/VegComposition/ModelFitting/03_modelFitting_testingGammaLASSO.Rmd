...
saveRDS(modDat_1_s, file = "./models/scaledModelInputData_GammaGLM.rds")
```
...
if (removeAllAnoms) { ## if the model parameters indicate that we don't want any anomalies... 
 prednames_new <- c ("tmean" , "I(log(prcp+1))",  "I(log(prcp_dry+1))",  #"prcpTempCorr",  
  ...
  #annual water deficit anomaly)
  #plot(density(log(X[,"carbon"]+1)))
  
 ## break predictors into climate, weather, and soils 
  prednames_clim <- prednames_new[which(prednames_new %in% c("tmin" , "tmax",  "tmean",
     "I(log(prcp_dry+1))", "t_warm" , "t_cold" , "prcp_wet" , "I(log(prcp_dry+1))" , "prcp_seasonality", "prcpTempCorr", "abvFreezingMonth" , "isothermality" , "I(log(annWatDef+2))" , "annWetDegDays",  "VPD_mean",  "VPD_max",  "VPD_min" , "VPD_max_95", "annWatDef_95", "annWetDegDays_5",  "frostFreeDays_5", "frostFreeDays"))]
...
# by hand for now
  prednames_new <- c ("tmean" , "I(log(prcp+1))",  "I(log(prcp_dry+1))",  "prcpTempCorr",  "isothermality" , "I(log(annWatDef+2))"  , "sand" ,  "coarse" ,  "I(log(carbon+1))",  "AWHC",                    "tmin_anom",  "tmax_anom" , "t_warm_anom" ,  "prcp_wet_anom",  "precp_dry_anom" ,  "prcp_seasonality_anom",   "prcpTempCorr_anom" , "aboveFreezingMonth_anom", "isothermality_anom",  #"annWatDef_anom", 
                      "annWetDegDays_anom",      "VPD_min_anom")
 ...
 ## break predictors into climate, weather, and soils 
  ...
  # get a text string of the possible untransformed model terms
  mod_string <- paste(response, "~", paste0(prednames_new, collapse = "+"))
...
if (run == TRUE) {
  ...
  my_folds <- as.numeric(as.factor(train_eco))
    ...
    fit <- cv.glmnet(
     ...
     base::saveRDS(fit, paste0("../ModelFitting/models/gammaLASSO/", response, "_globalLASSOmod_gammaLogLink_",ecoregion,"_removeAnomalies", removeAllAnoms,".rds"))
...
##
## look at distributions of variables used in the glm fit w/ predictors identified in bestlambda LASSO
as.data.frame(X) %>% 
  select(-"(Intercept)") %>% 
  pivot_longer(cols = any_of(bestLambda_coef$VariableName), names_to = "variable", values_to = "values") %>% 
  ggplot() + 
  facet_wrap(~variable, scales = "free", ncol = 6) + 
  geom_density(aes(values))
##
...
## fit using betareg
  fit_glm_bestLambda <- fit_glm_bestLambda_quasiBETA <- glm(formula = f_glm_bestLambda,data = #X_dataFrame#
                                                              modDat_1_s
                                                            , family = stats::Gamma(link = "log") )
  ...
  fit_glm_se <- glm(formula = f_glm_1se, data = #X_dataFrame
                      modDat_1_s
                    , family =stats::Gamma(link = "log"))
  ...
  fit_glm_halfse <- glm(formula = f_glm_halfse, data = #X_dataFrame
                        modDat_1_s
                        , family = stats::Gamma(link = "log"))
  
  ## save models 
  if (trimAnom == TRUE) {
    saveRDS(fit_glm_bestLambda, paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_trimAnom_bestLambdaGLM.rds"))
  saveRDS(fit_glm_halfse, paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_trimAnom_halfSELambdaGLM.rds"))
  saveRDS(fit_glm_se, paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_trimAnom_oneSELambdaGLM.rds"))
  } else {
    saveRDS(fit_glm_bestLambda, paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_bestLambdaGLM.rds"))
  saveRDS(fit_glm_halfse, paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_halfSELambdaGLM.rds"))
  saveRDS(fit_glm_se, paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_oneSELambdaGLM.rds"))
  ...
  save(f_glm_halfse, mat_glmnet_halfse, halfseLambda_coef,
              ...
              file = paste0("./models/interimModelFittingObjects_", response, "_", ecoregion, "_removeAnomalies", removeAllAnoms,"_gammaReg.rds" ))
  } else {
    # read in LASSO object
    fit <- readRDS(paste0("../ModelFitting/models/gammaLASSO/", response, "_globalLASSOmod_gammaLogLink_",ecoregion,"_removeAnomalies", removeAllAnoms,".rds"))
    
    # read in R objects having to do w/ model fitting 
    load(file = paste0("./models/interimModelFittingObjects_", response, "_", ecoregion, "_removeAnomalies", removeAllAnoms,"_gammaReg.rds" ))
      
    if (trimAnom == TRUE) {
    fit_glm_bestLambda <- readRDS( paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_trimAnom_bestLambdaGLM.rds"))
  fit_glm_halfse <- readRDS( paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_trimAnom_halfSELambdaGLM.rds"))
 fit_glm_se <- readRDS( paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_trimAnom_oneSELambdaGLM.rds"))
  } else {
    fit_glm_bestLambda <- readRDS( paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_bestLambdaGLM.rds"))
  fit_glm_halfse <- readRDS( paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_halfSELambdaGLM.rds"))
  fit_glm_se <- readRDS( paste0("./models/gammaLASSO/",response,"_",ecoregion, "_noTLP_",removeTLP,"_removeAnomalies", removeAllAnoms,"_oneSELambdaGLM.rds"))
...
optimal_pred <- predict.glm(fit_glm_bestLambda, newdata = modDat_1_s#newdata_DF
                            , type = "response")
...
#optimal_pred_betareg <- betareg:::predict.betareg(object = fit_glm_bestLambda_BETAREG, newdata = newdata_DF, type = "response")
  ...
  optimal_pred_1se <-  predict.glm(fit_glm_se, newdata = modDat_1_s#newdata_DF
                                   , type = "response")
  optimal_pred_halfse <- predict.glm(fit_glm_halfse, newdata = modDat_1_s#newdata_DF
                                     , type = "response")
...
```{r model evaluation figures, eval=TRUE, fig.width=9, fig.height=7}
...
x <- Gamma(link = "log")
  ...
  #                                  predVector[95] * X[,names(predVector[95])] + predVector[96] * X[,names(predVector[96])] + predVector[97] * X[,names(predVector[97])] + predVector[98] * X[,names(predVector[98])] + predVector[99] * X[,names(predVector[99])] +predVector[100] * X[,names(predVector[100])] + predVector[101] * X[,names(predVector[101])] + predVector[102] * X[,names(predVector[102])] + predVector[103] * X[,names(predVector[103])] + predVector[104] * X[,names(predVector[104])] + predVector[105] * X[,names(predVector[105])] + 
    ...
    # predVector[145] * X[,names(predVector[145])] + predVector[146] * X[,names(predVector[146])] + predVector[147] * X[,names(predVector[147])] + predVector[148] * X[,names(predVector[148])] + predVector[149] * X[,names(predVector[149])] 
  #)
 ...
 # plot(density(y), col = "black", ylim = c(0,.05),#xlim = c(-2, 2), 
...
plot(density(fullModOut$modelPredictions$obs), 
     ...
     main = c("black = observed values 
              red = preds. from 'best lambda' glm; orange = preds. from '1/2SE lambda' glm 
              green = preds. from '1se lambda' glm; blue = preds. from LASSO w/ best lambda")) #\n orange = predictions for '1/2se' lambda model \n green = predictions from '1se' lambda model "))
...
Note that if the model predicts values > 100, we snap those values to 100 here
...
pred_glm1_sf2 <- pred_glm1_sf2 %>% 
  mutate(Peak1 = replace(Peak1, (TotalTreeCover_pred < densOfPreds_max[1] + 3) & (TotalTreeCover_pred > densOfPreds_max[1] - 3), TRUE))
...
pred_glm1_sf2 <- pred_glm1_sf2 %>% 
  mutate(Peak2 = replace(Peak2, (TotalTreeCover_pred < densOfPreds_max[2] + 3) & (TotalTreeCover_pred > densOfPreds_max[2] - 3), TRUE))
...
labs(title = paste0("Location of model predictions that are in the bimodal 
                    ...
                    This shows predictions that are +/- 3 from the 
                    rounded value of the peak"),
     subtitle =  "purple = values near peak 1; 
     orange = values near peak 2")  +
  scale_fill_gradient2(low = "brown",
                       mid = "wheat",
                       high = "darkgreen" , 
                       midpoint = 50,   na.value = "white",
...
if(trimAnom == TRUE) {
saveRDS(coefs, file = paste0("./models/modelCoefficients_trimAnom_", ecoregion, "_", response,"_removeAnomalies", removeAllAnoms,"GammaLASSO.rds"))
saveRDS(uniqueCoeffs, file = paste0("./models/modelMetrics_trimAnom_", ecoregion, "_", response,"_removeAnomalies", removeAllAnoms,"GammaLASSO.rds"))
} else {
saveRDS(coefs, file = paste0("./models/modelCoefficients_", ecoregion, "_", response,"_removeAnomalies", removeAllAnoms,"GammaLASSO.rds"))
saveRDS(uniqueCoeffs, file = paste0("./models/modelMetrics_", ecoregion, "_", response,"_removeAnomalies", removeAllAnoms,"GammaLASSO.rds"))
}
# make a table
