...
We've fit models to multiple different functional groups, and want to both compare their performance across different iterations, 
...
grassShrub_cam <- readRDS("./models/modelCoefficients_shrubGrass_CAMCover.rds")
forest_cam <- readRDS("./models/modelCoefficients_forest_CAMCover.rds")
...
grassShrub_cam_trimAnoms <- readRDS("./models/modelCoefficients_trimAnom_shrubGrass_CAMCover.rds")
forest_cam_trimAnoms <- readRDS("./models/modelCoefficients_trimAnom_forest_CAMCover.rds")
...
modMetrics_grassShrub_cam <- readRDS("./models/modelMetrics_shrubGrass_CAMCover.rds")
modMetrics_forest_cam <- readRDS("./models/modelMetrics_forest_CAMCover.rds")
# with reduction of anomalies 
...
modMetrics_grassShrub_cam_trimAnom <- readRDS("./models/modelMetrics_trimAnom_shrubGrass_CAMCover.rds")
modMetrics_forest_cam_trimAnom <- readRDS("./models/modelMetrics_trimAnom_forest_CAMCover.rds")
```
...
knitr::kable(format = "html", modMetrics_grassShrub_shrub_trimAnom,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "grass/shrub - Shrub Cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))


knitr::kable(format = "html", modMetrics_grassShrub_bareGround_trimAnom,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "grass/shrub - Bare ground Cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
knitr::kable(format = "html", modMetrics_forest_shrub_trimAnom,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "forest - Shrub Cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

knitr::kable(format = "html", modMetrics_forest_bareGround_trimAnom,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "forest - Bare ground Cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

#CAM
knitr::kable(format = "html", modMetrics_grassShrub_cam_trimAnom,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "grass/shrub- CAM cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

knitr::kable(format = "html", modMetrics_forest_cam_trimAnom,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "forest - CAM cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
  
# Predictions with contemporary and modeled future climate data
...
### In this iteration, using data from: MACA v2 Livneh (6-7km grid), CanESM2 model, rcp4.5 (2046-2065)
```{r, eval = FALSE}
# MACA data from: https://climate.northwestknowledge.net/MACA/data_portal.php
## read in tmin data 
tmin <- rast("../../../Data_raw/macaClimateProjections/Data/macav2livneh_tasmin_CanESM2_r1i1p1_rcp45_2046_2065_CONUS_monthly.nc") %>% 
  terra::project(y = test_rast) # reproject to match the data used for model fitting
names(tmin) <- terra::time(tmin)

# get points to subsample
points <- terra::crds(tmin)
pointsSamp <- points[sample(x = 1:nrow(points), size = 50000, replace = FALSE),] %>% 
  terra::vect() %>% 
  terra::set.crs(crs(test_rast))

# subsample tmin points to a data frame
tminPoints <- tmin %>% 
  terra::extract(y = pointsSamp, xy = TRUE)
# get month and year data
tminPoints_temp <- tminPoints %>% 
  #slice_sample(n = 10) %>% 
  pivot_longer(cols = `2046-01-15`:`2065-12-15`, names_to = "date", values_to = "tmin_K") 

tminPoints_temp$Year <- as.numeric(str_split(tminPoints_temp$date, pattern = "-", simplify = TRUE)[,1])
tminPoints_temp$Month <- as.numeric(str_split(tminPoints_temp$date, pattern = "-", simplify = TRUE)[,2])

# convert temp in K to degrees C
tminPoints_temp$tmin_C <- tminPoints_temp$tmin_K - 273.15

# make into a wide format again
tminPoints <- tminPoints_temp %>% 
...
## read in tmax data 
tmax <- rast("../../../Data_raw/macaClimateProjections/Data/macav2livneh_tasmax_CanESM2_r1i1p1_rcp45_2046_2065_CONUS_monthly.nc") %>% 
  terra::project(y = test_rast) # reproject to match the data used for model fitting
names(tmax) <- terra::time(tmax)

# subsample tmax points to a data frame
tmaxPoints <- tmax %>% 
  terra::extract(y = pointsSamp, xy = TRUE)
# get month and year data
tmaxPoints_temp <- tmaxPoints %>% 
  #slice_sample(n = 10) %>% 
  pivot_longer(cols = `2046-01-15`:`2065-12-15`, names_to = "date", values_to = "tmax_K") 

tmaxPoints_temp$Year <- as.numeric(str_split(tmaxPoints_temp$date, pattern = "-", simplify = TRUE)[,1])
tmaxPoints_temp$Month <- as.numeric(str_split(tmaxPoints_temp$date, pattern = "-", simplify = TRUE)[,2])

# convert temp in K to degrees C
tmaxPoints_temp$tmax_C <- tmaxPoints_temp$tmax_K - 273.15

# make into a wide format again
tmaxPoints <- tmaxPoints_temp %>% 
  pivot_wider(id_cols = c(x, y, Year), 
              names_from = Month, 
              values_from = tmax_C, 
...
## read in precip data 
prcp <- rast("../../../Data_raw/macaClimateProjections/Data/macav2livneh_pr_CanESM2_r1i1p1_rcp45_2046_2065_CONUS_monthly.nc") %>% 
  terra::project(y = test_rast) # reproject to match the data used for model fitting
names(prcp) <- terra::time(prcp)

# subsample tmax points to a data frame
prcpPoints <- prcp %>% 
  terra::extract(y = pointsSamp, xy = TRUE)
# get month and year data
prcpPoints_temp <- prcpPoints %>% 
  #slice_sample(n = 10) %>% 
  pivot_longer(cols = `2046-01-15`:`2065-12-15`, names_to = "date", values_to = "prcp_mm") 

prcpPoints_temp$Year <- as.numeric(str_split(prcpPoints_temp$date, pattern = "-", simplify = TRUE)[,1])
prcpPoints_temp$Month <- as.numeric(str_split(prcpPoints_temp$date, pattern = "-", simplify = TRUE)[,2])

# make into a wide format again
prcpPoints <- prcpPoints_temp %>% 
  pivot_wider(id_cols = c(x, y, Year), 
              names_from = Month, 
              values_from = prcp_mm, 
...
climDat_monthly <- tminPoints %>% 
  left_join(tmaxPoints, by = c("x", "y", "Year")) %>% 
  left_join(prcpPoints, by = c("x", "y", "Year"))
```

## Calculate climate means and weather anomalies
```{r, eval = FALSE}
climVar <- climDat_monthly %>% 
  #slice(1:100) %>% 
...
climVar[is.na(climVar[,"durationFrostFreeDays"]),"durationFrostFreeDays"] <- 0
# if first month where tmin is above freezing is NA, change to 8 
climVar[is.na(climVar[,"aboveFreezing_month"]), "aboveFreezing_month"] <- 8
...
annMeans_30yrs <- slidingMetMeans(inDat = climVar,
                            start = as.numeric(2065-19), end = 2065)

names(annMeans_30yrs)[3:24] <- paste0(names(annMeans_30yrs)[3:24], "_CLIM")
annMeans_30yrs$End_CLIM <- 2065
# for last 3-year window
annMeans_3yrs <- slidingMetMeans(inDat = climVar,
                                 start = as.numeric(2065-3), end = 2065)

names(annMeans_3yrs)[3:24] <- paste0(names(annMeans_3yrs)[3:24], "_3yr")
annMeans_3yrs$End_3yr <- 2065
...
test$Start_CLIM <- 2065-19

# save intermediate data 
saveRDS(test, "../../../Data_processed/CoverData/IntermediateAnalysisFiles/ForecastedClimateDataForPredictions.rds")
...
# remove unnecessary soils variables 
  ...
  #   geom_point(data = soils_final[1:100,], aes(x, y), col = "red")
  
  
# add soils to climate data
forecastClimSoilsDat <- climDat %>% 
  cbind(soils_final, by = c("x", "y"))


## save the data
saveRDS(forecastClimSoilsDat, "../../../Data_processed/CoverData/IntermediateAnalysisFiles/Final_ForecastedClimateDataAndSoilsDataForPredictions.rds")
```

Prepare forecasted climate data for use in models 
```{r}

forecastClimSoilsDat <- readRDS("../../../Data_processed/CoverData/IntermediateAnalysisFiles/Final_ForecastedClimateDataAndSoilsDataForPredictions.rds")
forecastClimSoilsDat <- forecastClimSoilsDat[,-c(89,90)]
# rename
forecastClimSoilsDat <- forecastClimSoilsDat %>% 
...
# apply the scaling factors to the contemporary climate data 
namesToScale <- forecastClimSoilsDat %>% 
  select(tmin:frostFreeDays, tmean_anom:frostFreeDays_anom, soilDepth:AWHC) %>% 
  names()

forecastClimSoilsDat_scaled <- map(namesToScale, .f = function(x) {
  x_new <- (forecastClimSoilsDat[,x] - scaleParams[,paste0(x, "_s")]$`scaled:center`)/scaleParams[,paste0(x, "_s")]$`scaled:scale`
  return(data.frame(x_new))
}) %>% 
  purrr::list_cbind()
names(forecastClimSoilsDat_scaled) <- paste0(namesToScale, "_s")

forecastClimSoilsDatPred <- forecastClimSoilsDat %>% 
  select(x:Year) %>% 
  cbind(forecastClimSoilsDat_scaled)
names(forecastClimSoilsDatPred)[4:53] <- str_remove(names(forecastClimSoilsDatPred)[4:53], pattern = "_s$")
...
forecastClimSoilsDat_long <- forecastClimSoilsDatPred %>% 
...
modDat_long <- modDat_1_s %>% 
  ...
  rename(x = x.x, y = y.y) %>% 
  select(x, y, variableName, values, source)
allDat_long <- forecastClimSoilsDat_long %>% 
  rbind(modDat_long)
...
## Predict for grass/shrub total herbaceous cover
...
# read in model objects (is the trim anomaly version)
  oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/TotalHerbaceousCover_shrubGrassnoTLP_FALSE_trimAnom_oneSELambdaGLM.rds")
```

Print the **Best Lambda** model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = grassShrub_totalHerb_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "TotalHerbaceousCover")
ModelSpec_oneSElambda <- getModelStatement(coefficientTable = grassShrub_totalHerb_trimAnoms,
...
This is the model equation if the inputs are *not* scaled: 
```{r}
...
# predict w/ 1SE lambda model
oneSELambda_predict <- makePredictions(predictionDF = climDatPred, 
                                                           modelObject = oneSELambdaMod)

# predict with best SE lambda model w/ forecasted climate data
bestLambda_predictFuture <- makePredictions(predictionDF = forecastClimSoilsDatPred, 
                                                           modelObject = bestLambdaMod)

# predict with 1SE lambda model w/ forecasted climate data
oneSELambda_predictFuture <- makePredictions(predictionDF = forecastClimSoilsDatPred, 
                                                           modelObject = oneSELambdaMod)
...
## map oneSE lambda predictions
plotObs_oneSE <- oneSELambda_predict %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObs_oneSE_2 <- plotObs_oneSE %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )

## map best SE lambda predictions for the future
plotObs_bestLambdaFuture <- bestLambda_predictFuture %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObs_bestLambdaFuture_2 <- plotObs_bestLambdaFuture %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )
## map 1 SE lambda predictions for the future
plotObs_oneSELambdaFuture <- oneSELambda_predictFuture %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObs_oneSELambdaFuture_2 <- plotObs_oneSELambdaFuture %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )
...
map_oneSE <- ggplot() +
geom_spatraster(data = plotObs_oneSE_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_2)),fill=NA ) +
labs(title = paste0("Predictions of totalHerbaceousCover in the 
                    grass/shrub ecoregion"),
      subtitle = "oneSE Lambda model") +
  scale_fill_gradient2(low = "brown",
                       mid = "wheat" ,
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
...
map_bestlambda_future <- ggplot() +
geom_spatraster(data = plotObs_bestLambdaFuture_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_2)),fill=NA ) +
labs(title = paste0("Predictions of totalHerbaceousCover in the 
                    grass/shrub ecoregion"),
...
map_1SElambda_future <- ggplot() +
geom_spatraster(data = plotObs_oneSELambdaFuture_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_2)),fill=NA ) +
labs(title = paste0("Predictions of totalHerbaceousCover in the 
                    grass/shrub ecoregion"),
...
hist_oneSE <- ggplot(oneSELambda_predict) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  geom_density(data = oneSELambda_predict[oneSELambda_predict$newRegion == "dryShrubGrass",],
               aes(x = modelPreds), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
  ylab("frequency")

hist_bestlambdaFuture <-  ggplot(bestLambda_predictFuture) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  geom_density(data = oneSELambda_predict[oneSELambda_predict$newRegion == "dryShrubGrass",],
               aes(x = modelPreds), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
...
hist_oneSElambdaFuture<-  ggplot(oneSELambda_predictFuture) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  geom_density(data = oneSELambda_predict[oneSELambda_predict$newRegion == "dryShrubGrass",],
               aes(x = modelPreds), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
...
ggarrange(map, map_oneSE, hist, hist_oneSE, heights = c(3,1), ncol = 2, nrow = 2) %>% 
  annotate_figure(fig.lab = "Model Predictions with Contemporary Climate Data", fig.lab.size = 20)
ggarrange(map_bestlambda_future, map_1SElambda_future, 
          hist_bestlambdaFuture, hist_oneSElambdaFuture, heights = c(3,1), ncol = 2, nrow = 2)  %>% 
  annotate_figure(fig.lab = "Model Predictions with Forecasted Climate Data", fig.lab.size = 20)
```

## Predict for grass/shrub shrub

Read in the model objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/ShrubCover_shrubGrassnoTLP_FALSE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
  oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/ShrubCover_shrubGrassnoTLP_FALSE_trimAnom_oneSELambdaGLM.rds")
```

Print the model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = grassShrub_shrub_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "ShrubCover")
ModelSpec_oneSElambda <- getModelStatement(coefficientTable = grassShrub_shrub_trimAnoms,
                                                    modelName <- "coefficientValue_1seLambda", 
                                                    responseVar <- "ShrubCover")
...
hist_obs <- ggplot(modDat_1_s) + 
  ...
  geom_density(data = modDat_1_s[modDat_1_s$newRegion == "dryShrubGrass",],
               aes(x = ShrubCover), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
  ggtitle("Pink = predictions within 
          the focal ecoregion") +
  ylab("frequency")
...
## Predict for grass/shrub bare ground

Read in the model objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/BareGroundCover_shrubGrassnoTLP_FALSE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
  oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/BareGroundCover_shrubGrassnoTLP_FALSE_trimAnom_oneSELambdaGLM.rds")
```

Print the model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = grassShrub_bareGround_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "BareGroundCover")
ModelSpec_oneSElambda <- getModelStatement(coefficientTable = grassShrub_bareGround_trimAnoms,
                                                    modelName <- "coefficientValue_1seLambda", 
                                                    responseVar <- "BareGroundCover")

```

This is **best Lambda** the model equation if the inputs *are* scaled: 
```{r}
(ModelSpec_bestLambda$scaledInputVars_ModelStatement)
```
...
hist_obs <- ggplot(modDat_1_s) + 
  ...
  geom_density(data = modDat_1_s[modDat_1_s$newRegion == "dryShrubGrass",],
               aes(x = BareGroundCover), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
  ggtitle("Pink = predictions within 
          the focal ecoregion") +
  ylab("frequency")
...
## Predict for grass/shrub Total Tree cover (Texas-Lousiana Coastal Plain removed... but can change later)
Read in the best objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/TotalTreeCover_grassShrub_noTLP_TRUE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
  oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/TotalTreeCover_grassShrub_noTLP_TRUE_trimAnom_halfSELambdaGLM.rds")
```

Print the model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = grassShrub_totalTree_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
...
## Predict for grass/shrub CAM cover 
Read in the best objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/CAMCover_shrubGrass_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
  oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/CAMCover_shrubGrass_noTLP_FALSE_trimAnom_halfSELambdaGLM.rds")
```

Print the model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = grassShrub_cam_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
...
hist_obs <- ggplot(modDat_1_s) + 
  ...
  geom_density(data = modDat_1_s[modDat_1_s$newRegion == "dryShrubGrass",],
               aes(x = CAMCover), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
  ggtitle("Pink = predictions within 
          the focal ecoregion") +
  ylab("frequency")
...
## Predict for forest total herbaceous
Read in the objects
```{r}
# read in model objects (is the trim anomaly version)
...
# read in model objects (is the trim anomaly version)
  oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/TotalHerbaceousCover_forest_noTLP_FALSE_trimAnom_halfSELambdaGLM.rds")
```
...
hist <- ggplot(bestLambda_predict) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  geom_density(data = bestLambda_predict[bestLambda_predict$newRegion %in% c("eastForest", "westForest"),],
               aes(x = modelPreds), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
  ylab("frequency")
...
hist_oneSE <- ggplot(oneSELambda_predict) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  geom_density(data = oneSELambda_predict[oneSELambda_predict$newRegion %in% c("eastForest", "westForest"),],
               aes(x = modelPreds), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
  ylab("frequency")
...
hist_obs <- ggplot(modDat_1_s) + 
  geom_density(aes(TotalHerbaceousCover), fill = "lightgrey", col = "darkgrey") + 
  geom_density(data = modDat_1_s[modDat_1_s$newRegion %in% c("eastForest", "westForest"),],
               aes(x = TotalHerbaceousCover), fill = "orchid", col = "orchid", alpha = .3) +
  xlab("Predicted Value") + 
  ggtitle("Pink = predictions within 
          the focal ecoregion") +
...
## Predict for forest total tree cover
Read in the objects
```{r}
# read in model objects (is the trim anomaly version)
...
# read in model objects (is the trim anomaly version)
oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/TotalTreeCover_forest_noTLP_FALSE_trimAnom_halfSELambdaGLM.rds")
```
...
## Predict for forest shrub cover
Read in the objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/ShrubCover_forest_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/ShrubCover_forest_noTLP_FALSE_trimAnom_halfSELambdaGLM.rds")
```

Print the model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = forest_shrub_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "ShrubCover")
ModelSpec_oneSElambda <- getModelStatement(coefficientTable = grassShrub_totalTree_trimAnoms,
                                                    modelName <- "coefficientValue_halfseLambda", 
...
## Predict for forest bare ground cover
Read in the objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/BareGroundCover_forest_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/BareGroundCover_forest_noTLP_FALSE_trimAnom_halfSELambdaGLM.rds")
```

Print the model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = forest_bareGround_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "BareGroundCover")
ModelSpec_oneSElambda <- getModelStatement(coefficientTable = forest_bareGround_trimAnoms,
                                                    modelName <- "coefficientValue_halfseLambda", 
                                                    responseVar <- "BareGroundCover")
...
## Predict for forest CAM cover
Read in the objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/CAMCover_forest_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/CAMCover_forest_noTLP_FALSE_trimAnom_halfseLambdaGLM.rds")
```

Print the model specification for this model (including scaling factors)
```{r}
ModelSpec_bestLambda <- getModelStatement(coefficientTable = forest_cam_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "CAMCover")
ModelSpec_oneSElambda <- getModelStatement(coefficientTable = forest_cam_trimAnoms,
                                                    modelName <- "coefficientValue_halfseLambda", 
                                                    responseVar <- "CAMCover")
...
## CAM cover, CONUS-wide
Read in the objects
```{r, eval = FALSE}
# read in model objects (is the trim anomaly version)
 bestLambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/CAMCover_CONUSs_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

# read in model objects (is the trim anomaly version)
oneSELambdaMod <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/CAMCover_CONUS_noTLP_FALSE_trimAnom_halfSELambdaGLM.rds")
```
...
ModelSpec_bestLambda <- getModelStatement(coefficientTable = CONUS_cam_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "CAMcover")
ModelSpec_oneSElambda <- getModelStatement(coefficientTable = CONUS_cam_trimAnoms,
                                                    modelName <- "coefficientValue_halfseLambda", 
                                                    responseVar <- "CAMcover")
...
Predict
```{r fig.width = 12, fig.height = 6, message=FALSE, eval = FALSE}
# predict w/ best SE lambda model
bestLambda_predict <- makePredictions(predictionDF = climDatPred, 
                                                           modelObject = bestLambdaMod)
# predict w/ 1SE lambda model
