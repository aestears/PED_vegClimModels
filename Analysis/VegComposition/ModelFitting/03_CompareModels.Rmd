...
## needle-leaved tree as proportion of total tree, CONUS-wide - best lambda model
Read in the objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod_CONUS_needleLeavedTree <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/ConifTreeCover_prop_CONUS_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

ModelSpec_bestLambdaMod_CONUS_needleLeavedTree <- getModelStatement(coefficientTable = CONUS_needleLeavedTree_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
...
This is the **best Lambda** model equation if the inputs *are* scaled: 
```{r}
(ModelSpec_bestLambdaMod_CONUS_needleLeavedTree$scaledInputVars_ModelStatement)
```

This is the **best Lambda** model equation if the inputs are *not* scaled: 
```{r}
(ModelSpec_bestLambdaMod_CONUS_needleLeavedTree$unscaledInputVars_scaledModelStatement)
```

Predict
```{r fig.width = 16, fig.height = 18, message = FALSE}
# predict w/ best SE lambda model
bestLambda_CONUS_needleLeavedTree_predict <- makePredictions(predictionDF = climDatPred, 
                                                           modelObject = bestLambdaMod_CONUS_needleLeavedTree)
# predict with best SE lambda model w/ forecasted climate data
bestLambda_CONUS_needleLeavedTree_predictFuture_1 <- makePredictions(predictionDF = forecastClimSoilsDatPred_1, 
                                                           modelObject = bestLambdaMod_CONUS_needleLeavedTree)
# predict with best SE lambda model w/ forecasted climate data
bestLambda_CONUS_needleLeavedTree_predictFuture_2 <- makePredictions(predictionDF = forecastClimSoilsDatPred_2, 
modelObject = bestLambdaMod_CONUS_needleLeavedTree)
```

## broad-leaved tree as proportion of total tree, CONUS-wide - best lambda model
Read in the objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod_CONUS_broadLeavedTree <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/AngioTreeCover_prop_CONUS_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

ModelSpec_bestLambdaMod_CONUS_broadLeavedTree <- getModelStatement(coefficientTable = CONUS_broadLeavedTree_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
...
This is the **best Lambda** model equation if the inputs *are* scaled: 
```{r}
(ModelSpec_bestLambdaMod_CONUS_broadLeavedTree$scaledInputVars_ModelStatement)
```

This is the **best Lambda** model equation if the inputs are *not* scaled: 
```{r}
(ModelSpec_bestLambdaMod_CONUS_broadLeavedTree$unscaledInputVars_scaledModelStatement)
```

Predict
```{r fig.width = 16, fig.height = 18, message = FALSE}
# predict w/ best SE lambda model
bestLambda_CONUS_broadLeavedTree_predict <- makePredictions(predictionDF = climDatPred, 
                                                           modelObject = bestLambdaMod_CONUS_broadLeavedTree)
# predict with best SE lambda model w/ forecasted climate data
bestLambda_CONUS_broadLeavedTree_predictFuture_1 <- makePredictions(predictionDF = forecastClimSoilsDatPred_1, 
                                                           modelObject = bestLambdaMod_CONUS_broadLeavedTree)
# predict with best SE lambda model w/ forecasted climate data
bestLambda_CONUS_broadLeavedTree_predictFuture_2 <- makePredictions(predictionDF = forecastClimSoilsDatPred_2, 
modelObject = bestLambdaMod_CONUS_broadLeavedTree)
```
...
names(bestLambda_CONUS_broadLeavedTree_predict)[57] <- "broadLeavedTree_percentage_pred"
names(bestLambda_CONUS_needleLeavedTree_predict)[57] <- "needleLeavedTree_percentage_pred"
...
level2_cover_preds_contemp <- bestLambda_CONUS_C3_predict %>% 
  ...
  left_join(bestLambda_CONUS_broadLeavedTree_predict) %>% 
  left_join(bestLambda_CONUS_needleLeavedTree_predict) 
...
level2_cover_preds_contemp <- level2_cover_preds_contemp %>% 
  mutate("sumFromModels_totalHerbaceousCover" = C3_percentage_pred +  C4_percentage_pred + forb_percentage_pred, 
         "sumFromModels_totalTreeCover" = broadLeavedTree_percentage_pred + needleLeavedTree_percentage_pred) %>% 
  mutate(C3_percentage_scaled = (C3_percentage_pred/sumFromModels_totalHerbaceousCover)*100,
         ...
         broadLeavedTree_percentage_scaled = (broadLeavedTree_percentage_pred/sumFromModels_totalTreeCover)*100,
         needleLeavedTree_percentage_scaled = (needleLeavedTree_percentage_pred/sumFromModels_totalTreeCover)*100)
...
names(bestLambda_CONUS_broadLeavedTree_predictFuture_1)[55] <- "broadLeavedTree_percentage_pred"
names(bestLambda_CONUS_needleLeavedTree_predictFuture_1)[55] <- "needleLeavedTree_percentage_pred"
...
level2_cover_preds_future1 <- bestLambda_CONUS_C3_predictFuture_1 %>% 
  ...
  left_join(bestLambda_CONUS_broadLeavedTree_predictFuture_1) %>% 
  left_join(bestLambda_CONUS_needleLeavedTree_predictFuture_1) 
...
names(bestLambda_CONUS_broadLeavedTree_predictFuture_2)[55] <- "broadLeavedTree_percentage_pred"
names(bestLambda_CONUS_needleLeavedTree_predictFuture_2)[55] <- "needleLeavedTree_percentage_pred"
...
level2_cover_preds_future2 <- bestLambda_CONUS_C3_predictFuture_2 %>% 
  ...
  left_join(bestLambda_CONUS_broadLeavedTree_predictFuture_2) %>% 
  left_join(bestLambda_CONUS_needleLeavedTree_predictFuture_2) 
...
plotObs <- level2_cover_preds_contemp %>% 
  ...
  terra::set.crs(crs(test_rast)) %>% 
  terra::rasterize(y = test_rast, 
                   field = "broadLeavedTree_percentage_scaled", 
                   fun = mean, na.rm = TRUE)
# get the extent of this particular raster, and crop it accordingly
...
# get plot of observations
plotObservations_broadLeaved_proportion <- modDat_1_s %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("Long", "Lat")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObservations_broadLeaved_proportion_2 <- plotObservations_broadLeaved_proportion %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )
# make figures
...
map_obs_CONUS_broadLeavedTree <- ggplot() +
  geom_spatraster(data = plotObservations_broadLeaved_proportion_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_2)),fill=NA ) +
...
hist <- ggplot(level2_cover_preds_contemp) + 
  geom_density(aes(broadLeavedTree_percentage_scaled), fill = "lightgrey", col = "darkgrey") + 
   xlab("Predicted Value") + 
  ylab("frequency")
...
## calculate residuals for contemporary prediction
# (observed - predicted)
resids <-  plotObservations_broadLeaved_proportion_2 - plotObs_2 
map_resids_trimAnoms <- ggplot() +
geom_spatraster(data = resids) + 
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(resids)),fill=NA )  + 
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Resids. (obs. - pred.) from the 
                      CONUS-wide model of broad-leaved tree cover"),
     subtitle = "using predictions from the Trim Anomalies 
     bestLambda model with contemporary climate data") +
  scale_fill_gradient2(low = "red",
                       mid = "white" ,
...
geom_spatraster(data = predDeltas_model1) + 
  ...
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Future Climate Model Deltas for 
                      CONUS-wide model of broad-leaved tree covver; (models with 
                      predictions with modeled climate data from model BNU-ESM 
                      model - models with predictions from 
                      contemporary climate data)"),
     subtitle = "using predictions from the Trim Anomalies 
...
geom_spatraster(data = predDeltas_model2) + 
  ...
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Future Climate Model Deltas for
  CONUS-wide model of broad-leaved tree covver; 
  (models with predictions with modeled 
  climate data from model IPSL-CM5A-MR 
  model- models with predictions from 
  contemporary climate data)"),
...
## conglomerate figure
  ...
  ggarrange(map_obs_CONUS_broadLeavedTree, hist_obs, heights = c(3,1), ncol = 1, nrow  = 2)
  ...
  # plot model forecasts with model that does not exclude anomalies
       ggarrange(map, map_bestlambda_future1, map_bestlambda_future2,
            ...
            hist_trimAnoms_resids, hist_deltas_model1, hist_deltas_model2,
            heights = c(3,1), ncol = 3, nrow = 4) %>% 
  annotate_figure(fig.lab = "Model Predictions of broadLeavedTree with Contemporary and Forecasted Climate Data", fig.lab.size = 20)
```
...
plotObs <- level2_cover_preds_contemp %>% 
  ...
  terra::set.crs(crs(test_rast)) %>% 
  terra::rasterize(y = test_rast, 
                   field = "needleLeavedTree_percentage_scaled", 
                   fun = mean, na.rm = TRUE)
# get the extent of this particular raster, and crop it accordingly
...
hist <- ggplot(level2_cover_preds_contemp) + 
  geom_density(aes(needleLeavedTree_percentage_scaled), fill = "lightgrey", col = "darkgrey") + 
   xlab("Predicted Value") + 
  ylab("frequency")
...
geom_spatraster(data = resids) + 
  ...
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Resids. (obs. - pred.) from the 
                      CONUS-wide model of needle-leaved tree cover"),
     subtitle = "using predictions from the Trim Anomalies 
     bestLambda model with contemporary climate data") +
  scale_fill_gradient2(low = "red",
                       mid = "white" ,
...
geom_spatraster(data = predDeltas_model1) + 
  ...
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Future Climate Model Deltas for 
                      CONUS-wide model of needle-leaved tree covver; (models with 
                      predictions with modeled climate data from model BNU-ESM 
                      model - models with predictions from 
                      contemporary climate data)"),
     subtitle = "using predictions from the Trim Anomalies 
...
## conglomerate figure
  ...
  # plot model forecasts with model that does not exclude anomalies
       ggarrange(map, map_bestlambda_future1, map_bestlambda_future2,
            ...
            hist_trimAnoms_resids, hist_deltas_model1, hist_deltas_model2,
            heights = c(3,1), ncol = 3, nrow = 4) %>% 
  annotate_figure(fig.lab = "Model Predictions of needleLeavedTree with Contemporary and Forecasted Climate Data", fig.lab.size = 20)
```
...
preds_contemp <- preds_contemp_cover1 %>% 
 left_join(preds_byHand %>% select(x:y, prob_forest, prob_GrassShrub))

preds_future_1 <- preds_future_1_cover1 %>% 
 left_join(preds_future1_byHand %>% select(x:y, prob_forest, prob_GrassShrub))

preds_future_2 <- preds_future_2_cover1 %>% 
 left_join(preds_future2_byHand %>% select(x:y, prob_forest, prob_GrassShrub))
...
preds_contemp <- preds_contemp %>% 
 mutate(C3_cover_finalScaled = totalHerb_cover_finalScaled *(C3_percentage_scaled/100),
        ...
        broadLeaved_cover_finalScaled = totalTree_cover_finalScaled *(broadLeavedTree_percentage_scaled/100),
        needleLeaved_cover_finalScaled = totalTree_cover_finalScaled *(needleLeavedTree_percentage_scaled/100)) 
...
plotObservations_broadLeaved_actualCover <-  plotObservations_F_totTree_2 * (plotObservations_broadLeaved_proportion_2/100)
