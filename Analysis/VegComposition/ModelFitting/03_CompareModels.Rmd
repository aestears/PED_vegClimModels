...
```{r}
# read in model objects (is the trim anomaly version)
   bestLambdaMod_CONUS_CAM <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/CAMCover_CONUS_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

ModelSpec_bestLambda_CONUS_CAM <- getModelStatement(coefficientTable = CONUS_cam_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
...
This is the **best Lambda** model equation if the inputs *are* scaled: 
```{r}
(ModelSpec_bestLambda_CONUS_CAM$scaledInputVars_ModelStatement)
```

This is the **best Lambda** model equation if the inputs are *not* scaled: 
```{r}
(ModelSpec_bestLambda_CONUS_CAM$unscaledInputVars_scaledModelStatement)
```

Predict
```{r fig.width = 16, fig.height = 18, message = FALSE}
# predict w/ best SE lambda model
bestLambda_CONUS_CAM_predict <- makePredictions(predictionDF = climDatPred, 
                                                           modelObject = bestLambdaMod_CONUS_CAM)
# predict with best SE lambda model w/ forecasted climate data
bestLambda_CONUS_CAM_predictFuture_1 <- makePredictions(predictionDF = forecastClimSoilsDatPred_1, 
                                                           modelObject = bestLambdaMod_CONUS_CAM)
# predict with best SE lambda model w/ forecasted climate data
bestLambda_CONUS_CAM_predictFuture_2 <- makePredictions(predictionDF = forecastClimSoilsDatPred_2, 
                                                           modelObject = bestLambdaMod_CONUS_CAM)

# predict w/ best model
plotObs <- bestLambda_CONUS_CAM_predict %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
## map best SE lambda predictions for the future model #1
plotObs_bestLambdaFuture1 <- bestLambda_CONUS_CAM_predictFuture_1 %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
## map best SE lambda predictions for the future model #2
plotObs_bestLambdaFuture2 <- bestLambda_CONUS_CAM_predictFuture_2 %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
hist <- ggplot(bestLambda_CONUS_CAM_predict) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
   xlab("Predicted Value") + 
  ylab("frequency")

hist_bestlambdaFuture1 <-  ggplot(bestLambda_CONUS_CAM_predictFuture_1) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
   xlab("Predicted Value") + 
  ylab("frequency")

hist_bestlambdaFuture2 <-  ggplot(bestLambda_CONUS_CAM_predictFuture_2) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
   xlab("Predicted Value") + 
  ylab("frequency")
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotObs_future1, na.rm = TRUE)
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotObs_future2, na.rm = TRUE)
...
# CONUS CAM - best lambda model  (bestLambdaMod_CONUS_CAM)
# predict with contemporary data
#bestLambda_CONUS_CAM_predict
names(bestLambda_CONUS_CAM_predict)[57] <- "predContemp_CONUS_CAM"
# predict with modeled future climate data v1
#bestLambda_CONUS_CAM_predictFuture_1
names(bestLambda_CONUS_CAM_predictFuture_1)[55] <- "predFuture1_CONUS_CAM"
# predict with modeled future climate data v2
#bestLambda_CONUS_CAM_predictFuture_2
names(bestLambda_CONUS_CAM_predictFuture_2)[55] <- "predFuture2_CONUS_CAM"
...
preds_contemp_cover1 <- bestLambda_GS_totHerb_predict %>% 
  ...
  left_join(bestLambda_CONUS_CAM_predict)  %>% 
...
preds_future_1_cover1 <-  bestLambda_GS_totHerb_predictFuture_1 %>% 
  ...
  left_join(bestLambda_CONUS_CAM_predictFuture_1)  %>% 
...
preds_future_2_cover1 <- bestLambda_GS_totHerb_predictFuture_2 %>% 
  ...
  left_join(bestLambda_CONUS_CAM_predictFuture_2)  %>% 
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotPreds_totHerbaceous, na.rm = TRUE)
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotPreds_totTree, na.rm = TRUE)
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotPreds_totHerbaceous_future1, na.rm = TRUE)
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotPreds_totTree_future1, na.rm = TRUE)
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotPreds_totHerbaceous_future2, na.rm = TRUE)
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotPreds_totTree_future2, na.rm = TRUE)
...
## contemporary climate data
preds_contemp <- preds_contemp %>% 
  mutate(totalCover =  predContemp_CONUS_CAM + predContemp_CONUS_shrub + predContemp_CONUS_bareGround + totHerb_synth + totTree_synth)  %>% # calculate total cover "%" in a location
  # scale so that the cover classes sum to 100 
  mutate(CAM_cover_finalScaled = predContemp_CONUS_CAM/totalCover * 100,
         shrub_cover_finalScaled = predContemp_CONUS_shrub/totalCover * 100,
...
preds_future_1 <- preds_future_1 %>% 
  mutate(totalCover =  predFuture1_CONUS_CAM + predFuture1_CONUS_shrub + predFuture1_CONUS_bareGround + totHerb_synth + totTree_synth)  %>% # calculate total cover "%" in a location
  # scale so that the cover classes sum to 100 
  mutate(CAM_cover_finalScaled = predFuture1_CONUS_CAM/totalCover * 100,
         shrub_cover_finalScaled = predFuture1_CONUS_shrub/totalCover * 100,
...
preds_future_2 <- preds_future_2 %>% 
  mutate(totalCover =  predFuture2_CONUS_CAM + predFuture2_CONUS_shrub + predFuture2_CONUS_bareGround + totHerb_synth + totTree_synth)  %>% # calculate total cover "%" in a location
  # scale so that the cover classes sum to 100 
  mutate(CAM_cover_finalScaled = predFuture2_CONUS_CAM/totalCover * 100,
         shrub_cover_finalScaled = predFuture2_CONUS_shrub/totalCover * 100,
...
contempScaledPreds_maps <- map(c("CAM_cover_finalScaled", "shrub_cover_finalScaled",
...
future1ScaledPreds_maps <- map(c("CAM_cover_finalScaled", "shrub_cover_finalScaled",
         "bareGround_cover_finalScaled",  "totalHerb_cover_finalScaled", 
         "totalTree_cover_finalScaled"), .f = function(x) {
           # make the raster
...
future2ScaledPreds_maps <- map(c("CAM_cover_finalScaled", "shrub_cover_finalScaled",
         "bareGround_cover_finalScaled",  "totalHerb_cover_finalScaled", 
         "totalTree_cover_finalScaled"), .f = function(x) {
           # make the raster
...
ggarrange(contempScaledPreds_maps[[4]], contempScaledPreds_maps[[5]], contempScaledPreds_maps[[1]], contempScaledPreds_maps[[2]], contempScaledPreds_maps[[3]], common.legend = TRUE, ncol = 3, nrow = 2)  %>% 
  annotate_figure(fig.lab = "Scaled model predictions of 'level 1' cover variables with contemporary data ", fig.lab.size = 20)


ggarrange(future1ScaledPreds_maps[[4]], future1ScaledPreds_maps[[5]], future1ScaledPreds_maps[[1]], future1ScaledPreds_maps[[2]], future1ScaledPreds_maps[[3]], common.legend = TRUE, ncol = 3, nrow = 2)  %>% 
...
ggarrange(future2ScaledPreds_maps[[4]], future2ScaledPreds_maps[[5]], future2ScaledPreds_maps[[1]], future2ScaledPreds_maps[[2]], future2ScaledPreds_maps[[3]], common.legend = TRUE, ncol = 3, nrow = 2)  %>% 
