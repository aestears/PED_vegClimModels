...
Once we generate predictions using each of these models, we transform the ecoregion-level predictions for total herbaceous cover and total tree cover into CONUS-wide predictions by scaling them according to predicted ecoregion probability from the ecoregion classification model.
...
contempPreds <- contempPreds %>% 
  mutate(
     absNLTrees_V1 = ((prob_grassShrub * percNLTree_GS) + (prob_Forest * percNLTree_F))/((prob_grassShrub * percBLTree_GS) + (prob_Forest * percBLTree_F) + ((prob_grassShrub * percNLTree_GS) + (prob_Forest * percNLTree_F))) * ((prob_grassShrub * absTotalTree_GS) + (prob_Forest * absTotalTree_F)) ,# option 1 (old approach)
         
         absNLTrees_V2 =  (prob_grassShrub * ((percNLTree_GS/(percBLTree_GS + percNLTree_GS)) * absTotalTree_GS)) + (prob_Forest * 
