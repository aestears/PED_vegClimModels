...
preds_byHand <- preds_byHand %>% 
  mutate(pred_binary = pred,
    pred_binary = replace(pred_binary, pred_binary > 0.45226130653266, 1),
         pred_binary = replace(pred_binary, pred_binary <= 0.45226130653266, 0))
# ggplot(preds_byHand) +
...
relCover_B_Maps_contemp <- lapply(responseNames, 
                                      FUN = function(x) {
                                       ...
                                       ## contemporary absolute cover
                                        ...
                                        resp_hist <- ggplot() + 
                                          geom_density(aes(contempPreds[,x]), col = "black", fill = "darkgrey") + 
                                          xlab(paste0("Predictions of Total Relative Cover (non-tree) with contemporary data")) +
                                          theme_classic()
