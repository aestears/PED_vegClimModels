...
# Interpolate the continuous predictions and make binary using the identified threshold
## interpolation function
## set up function for linear interpolation - best lambda model
approxfun_forest <- stats::approxfun(x = c(0,  best_threshold_bestLambda, 1), # values we're interpolating *from*
...
preds_byHand <- preds_byHand %>% 
 ...
 # mutate(pred_interp = replace(pred_interp, list = pred < 0.136147, 0)) %>% 
   mutate(pred_binary = pred_interp,
    pred_binary = replace(pred_binary, pred_binary > 0.1, 1), # now, make the interpolated continuous model predictions binary
         pred_binary = replace(pred_binary, pred_binary <= 0.1, 0)) 
...
map_ecoRegion_current <- ggplot() +
  geom_raster(data = plotObs_2, aes(x = x, 
                   y = y, 
                   fill = as.character(mean)), maxcell = 50000000 ) + 
...
<!-- We used the binary predictions of forest occurrence shown above to identify the data we used to fit the 'ecoregion-level' cover models, but we also want to use the continuous predictions of forest probability to scale the cover model predictions to make them CONUS-wide. Because we use a "custom" threshold to convert the continuous probability predictions from the binomial model to binary classifications, we also need to scale the continuous predictions according to this custom threshold. We do this by performing linear interpolation of the continuous probability values above and below the threshold value, the results of which are shown below using both contemporary and forecasted climate data. We also incorporated a calibration step so that the model predicts locations that are completely unforested (P(forest) = 0) at the same rate as we observe unforested locations based on RAP data. -->
<!-- ```{r fig.width = 16, fig.height = 4, message = FALSE} -->
<!-- ## interpolation happened above  -->
<!-- ## Plot predictions with contemporary data -->
<!-- # make into a raster -->
<!-- plotObs <- preds_byHand %>%  -->
<!--          #drop_na(paste(response)) %>%  -->
...
<!--                     contemporary dayMet data (from 2023)"), -->
<!--      subtitle = c("Values are the continuous predictions within each 8kmx8km grid cell")) + -->
...
<!-- Below is a map of locations that the model predicts to have a continuous probability of forest that is equal to zero:  -->
<!-- ```{r fig.width = 16, fig.height = 4, message = FALSE} -->
<!-- ## interpolation happened above  -->
<!-- ## Plot predictions with contemporary data -->
<!-- plotObs_2_zero <- plotObs_2 -->
<!-- plotObs_2_zero[plotObs_2_zero$mean != 0] <- NA -->

<!-- map_ecoRegion_current_zero <- ggplot() + -->
<!--   geom_spatraster(data = plotObs_2_zero, maxcell = 50000000) +  -->
<!--   #geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) + -->
<!--   geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_2)),fill=NA ) + -->
<!-- labs(title = c("Locations where the forest probability predictions (interpolated)  -->
<!-- using  -->
<!-- contemporary dayMet data (from 2023) are 0")) + -->
<!--   scale_fill_gradient2(low = "purple", -->
<!--                        mid = "purple" , -->
<!--                        high = "purple" , limits = c(0,0),  na.value = "lightgrey") +  -->
<!--   xlim(st_bbox(plotObs_2)[c(1,3)]) +  -->
<!--   ylim(st_bbox(plotObs_2)[c(2,4)]) -->
...
<!-- plotObs_future1_2_zero <- plotObs_future1_2 -->
<!-- plotObs_future1_2_zero[plotObs_future1_2_zero$mean != 0] <- NA -->

<!-- map_ecoRegion_future1_zero <- ggplot() + -->
<!--   geom_spatraster(data = plotObs_future1_2_zero, maxcell = 50000000) +  -->
<!--   #geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) + -->
<!--   geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_future1_2)),fill=NA ) + -->
<!--   labs(title = c("Locations where the forest probability predictions (interpolated)  -->
<!-- using  -->
<!-- future climate data from the BNU-ESM model are 0")) + -->
<!--   scale_fill_gradient2(low = "purple", -->
<!--                        mid = "purple" , -->
<!--                        high = "purple" , limits = c(0,0),  na.value = "lightgrey") +  -->
<!--   xlim(st_bbox(plotObs_future1_2)[c(1,3)]) +  -->
...
<!-- plotObs_future2_2_zero <- plotObs_future2_2 -->
<!-- plotObs_future2_2_zero[plotObs_future2_2_zero$mean != 0] <- NA -->

<!-- map_ecoRegion_future2_zero <- ggplot() + -->
<!--   geom_spatraster(data = plotObs_future2_2_zero, maxcell = 50000000) +  -->
<!--   #geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) + -->
<!--   geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_future2_2)),fill=NA ) + -->
<!-- labs(title = paste0("Forest probability predictions (interpolated) using future  -->
<!-- climate data from the IPSL-CM5A-MR model")) + -->
<!--   labs(title = c("Locations where the forest probability predictions (interpolated)  -->
<!-- future  -->
<!-- climate data from the IPSL-CM5A-MR model are 0")) + -->
<!--   scale_fill_gradient2(low = "purple", -->
<!--                        mid = "purple" , -->
<!--                        high = "purple" , limits = c(0,0),  na.value = "lightgrey") +  -->
<!--   xlim(st_bbox(plotObs_future2_2)[c(1,3)]) +  -->
...
<!-- ## Plot the maps together -->
<!--   ggarrange(map_ecoRegion_current_zero, map_ecoRegion_future1_zero, map_ecoRegion_future2_zero, nrow = 1) %>%  -->
<!--   annotate_figure(fig.lab = "Model Predictions of Continuous P(Forest) == 0 - interpolated- with Contemporary and Forecasted Climate Data", fig.lab.size = 20,  -->
<!--                   bottom = text_grob("Note: Pink lines show the boundaries of contemporary grass/shrub and forest ecoregions, based on aggregated, EPA level 1 ecoregions")) -->
<!-- ``` -->
...
contempPreds <- totalHerb_CONUS_predsContemp  %>% 
  ...
  cbind(preds_byHand %>% select(pred_binary, pred_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "pred_interp") %>% 
...
predsFuture1 <- totalHerb_CONUS_predsFuture1  %>% 
  ...
  cbind(preds_future1_byHand %>% select(pred_binary, pred_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "pred_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
...
predsFuture2 <- totalHerb_CONUS_predsFuture2  %>% 
  ...
  cbind(preds_future2_byHand %>% select(pred_binary, pred_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "pred_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
...
makePredsCONUSwide <- function(dat) {
  ...
  dat2 <- dat %>% 
      mutate(prob_NonForest_b = as.integer(!as.logical(as.integer(prob_Forest_b)))) %>% 
  mutate(absTotalTree_GS_scaled = prob_NonForest_b * absTotalTree_GS,
         absTotalTree_F_scaled = prob_Forest_b * absTotalTree_F) %>% 
...
absoluteCoverMaps_contemp <- lapply(responseNames, 
                                      FUN = function(x) {
                                       ## contemporary absolute cover
                                        ...
                                        # make a histogram
                                        resp_hist <- ggplot(contempPreds %>% st_drop_geometry()) + 
                                        ...
                                        rast_deltas_future1 <- rast_deltas_future1 %>% 
                                          aggregate(fact = 8, fun = "mean", na.rm = TRUE)
                                        ...
                                        # temporarily aggregate to coarser raster for visualization purposes
                                        rast_deltas_future2 <- rast_deltas_future2 %>% 
                                          aggregate(fact = 8, fun = "mean", na.rm = TRUE)
...
preds_quantile <- totalHerb_CONUS_predsQuantile  %>% 
  ...
  cbind(preds_byHand_quantile %>% select(pred_binary, pred_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "pred_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
...
decilePredNames_totalTree<- c(str_remove_all(c(str_split(names(coefficients(totalTree_F)), ":", simplify = TRUE))[-1], pattern = regex("[I\\(]")) %>% 
...
## decile plot for data w/ all tree cover types using function (not by hand) 
...
quantPlot_totalTrees_NLBL <- add_dotplot_inset(quantPlot_totalTrees_NLBL, df = deciles_totTree_NLBL, response = c("TotalTreeCover", "absTotalTree_CONUS"), dfRaw = preds_quantile, add_smooth = TRUE, deciles = FALSE)

## quantile plot for total tree w/ all available data in forest regions
...
quantPlot_totalTrees_F <- decile_dotplot_pq(df = deciles_totTree_Forest, response= c("TotalTreeCover", "absTotalTree_CONUS"), IQR = FALSE,
                        CI = TRUE
                        ) + ggtitle("Absolute Total Tree Cover in Forest (model-defined): Quantile Plot")

quantPlot_totalTrees_F <- add_dotplot_inset(quantPlot_totalTrees_F, df = deciles_totTree_Forest, response = c("TotalTreeCover", "absTotalTree_CONUS"), dfRaw = preds_quantile, add_smooth = TRUE, deciles = FALSE)

## quantile plot for total tree w/ all available data in non-forest regions
...
quantPlot_totalTrees_noF <- decile_dotplot_pq(df = deciles_totTree_nonForest, response= c("TotalTreeCover", "absTotalTree_CONUS"), IQR = FALSE,
                        CI = TRUE
                        ) + ggtitle("Absolute Total Tree Cover in Non-Forest (model-defined): Quantile Plot")

quantPlot_totalTrees_noF <- add_dotplot_inset(quantPlot_totalTrees_noF, df = deciles_totTree_nonForest, response = c("TotalTreeCover", "absTotalTree_CONUS"), dfRaw = preds_quantile, add_smooth = TRUE, deciles = FALSE)
...
quantPlot_totalTrees_F_NLBL <- add_dotplot_inset(quantPlot_totalTrees_F_NLBL, df = deciles_totTree_Forest_NLBL, response = c("TotalTreeCover", "absTotalTree_CONUS"), dfRaw = preds_quantile, add_smooth = TRUE, deciles = FALSE)
...
## are there any locations where we have observations for NL tree and not BL tree? (we shouldn't) 
#sum(is.na(preds_quantile[!is.na(preds_quantile$NLTreeCover_abs), "BLTreeCover_abs"])) # as expected, no places where we have NL but not BL trees 
...
predsOverTime <- totalHerb_CONUS_predsOverTime  %>% 
  ...
  cbind(preds_byHand_time %>%  select(pred_binary, pred_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "pred_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
                                      ...
                                      FUN = function(x) {
                                       ...
                                       ## contemporary absolute cover
                                        ...
                                        mapResids <- ggplot() +
                                          ...
                                          geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rastTemp)) %>% 
                                                    st_crop(st_bbox(resids_rast)),fill=NA ) +
                                          labs(title = paste0("Residuals (observations - predicted relative cover) for ",x)) +
                                          scale_fill_gradient2(low = "red",
                                                               mid = "white" ,
                                                               high = "blue" , 
                                                               midpoint = 0,   na.value = "lightgrey",
                                        ...
                                        # calculate differences between contemporary preds and future 1 preds (deltas)
                                        rast_deltas_future1 <- resp_rast_future1_2 - resp_rast_2
                                        # for now, aggregate x8 for visualization purposes
                                        rast_deltas_future1 <- rast_deltas_future1 %>% 
                                          aggregate(fact = 8, fun = "mean", na.rm = TRUE)
                                        ...
                                        resp_map <- ggplot() +
                                          ...
                                          geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rastTemp)) %>%
                                                    st_crop(st_bbox(plotObs_2)),fill=NA ) +
                                                    labs(title = paste0("Predictions of Total Relative Cover (non-tree) with contemporary data")) +
                                        ...
                                        resp_map_future1 <- ggplot() +
                                          ...
                                          geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rastTemp)) %>%
                                                    st_crop(st_bbox(plotObs_2)),fill=NA ) +
                                                    labs(title = paste0("Future predictions of Total Relative Cover (non-tree) \n compared to contemporary predictions (future - contemprary)  \n - climate model: BNU-ESM")) +
                                          scale_fill_gradient2(low = "orange",
                                                               mid = "white" ,
                                                               high = "purple" , 
...
```{r rendering maps of relative cover, fig.height=60, fig.width=18}
ggarrange(
