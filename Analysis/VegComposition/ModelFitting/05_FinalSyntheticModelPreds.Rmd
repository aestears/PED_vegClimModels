...
climDat <- climDat_Temp %>% 
  ...
  #               x, y, soilDepth:totalAvailableWaterHoldingCapacity) %>% 
...
names(climDatPred)[2:52] <- str_remove(names(climDatPred)[2:52], pattern = "_s$")

## contemporary climate data for temporal analysis
climDat_timeTemp <- climDat_Temp#readRDS("../../../Data_processed/EcoRegion_climSoilData.rds")
...
names(climDatPred_tree)[2:52] <- str_remove(names(climDatPred_tree)[2:52], pattern = "_s$")
...
preds_byHand <- preds_byHand %>% 
  mutate(pred_binary = pred,
    pred_binary = replace(pred_binary, pred_binary > best_threshold_bestLambda, 1),
...
approxfun_pred <- approxfun(x = c(0,  0.45226130653266, 1), 
          y =c(0, 0.5, 1), 
          method = "linear")
...
contempPreds <- totalHerb_CONUS_predsContemp  %>% 
  ...
  cbind(preds_byHand %>% select(pred_binary, preds_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "preds_interp") %>% 
...
predsFuture1 <- totalHerb_CONUS_predsFuture1  %>% 
  ...
  cbind(preds_future1_byHand %>% select(pred_binary, preds_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "preds_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
...
predsFuture2 <- totalHerb_CONUS_predsFuture2  %>% 
  ...
  cbind(preds_future2_byHand %>% select(pred_binary, preds_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "preds_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
...
# ggplot(contempPreds) +
#   geom_point(aes(x = x, y = y, col = absTotalTree_CONUS))
# ggplot(contempPreds) +
#   geom_point(aes(x = x, y = y, col = absTotalTree_F_scaled))
# ggplot(contempPreds) +
#   geom_point(aes(x = x, y = y, col = absTotalTree_GS_scaled))
...
preds_quantile <- totalHerb_CONUS_predsQuantile  %>% 
  ...
  cbind(preds_byHand_quantile %>% select(pred_binary, preds_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "preds_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
...
# curtailing the tree deciles to be claculated only w/ data that also has NL and BL trees
testNames <- decilePredNames_totalTree[decilePredNames_totalTree %in% c(decilePredNames_NLTree[decilePredNames_NLTree %in% decilePredNames_BLTree])]
deciles_test <- deciles_NLtree[deciles_NLtree$name %in% testNames,] %>%
...
predsOverTime <- totalHerb_CONUS_predsOverTime  %>% 
  ...
  cbind(preds_byHand_time %>%  select(pred_binary, preds_interp)) %>% 
  rename(prob_Forest_b = "pred_binary",
         prob_Forest_c = "preds_interp") %>% 
  mutate(prob_grassShrub_b = 1 - prob_Forest_b,
         prob_grassShrub_c = 1 - prob_Forest_c)
