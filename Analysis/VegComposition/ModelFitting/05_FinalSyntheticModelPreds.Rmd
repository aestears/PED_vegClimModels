...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = prob_Forest_c)) + 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = absTotalHerb_CONUS)) + 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = percNLTree_scaled_F)) + 
  ggtitle("Prop. NL tree, scaled with Prop. BL tree to sum to 1, from model fit in Forest"), 
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  facet_wrap(~prob_Forest_b) + 
  geom_density(aes(x = percNLTree_scaled_F))+ 
  #geom_vline(aes(xintercept = .1), lty = 2) +
  ggtitle("Density of Prop. NL tree, scaled with Prop. BL tree to sum to 1, from model fit in Forest in \nforest (1) and non-forest (0)"), 
widths = c(1, .5)
) 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = percBLTree_scaled_F)) + 
  ggtitle("Prop. BL tree, scaled with Prop. NL tree to sum to 1, from model fit in Forest"), 
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  facet_wrap(~prob_Forest_b) + 
  geom_density(aes(x = percBLTree_scaled_F))+ 
  #geom_vline(aes(xintercept = .1), lty = 2) +
  ggtitle("Density of Prop. BL tree, scaled with Prop. NL tree to sum to 1, from model fit in Forest in \nforest (1) and non-forest (0)"), 
widths = c(1, .5)
) 
 ## step 7: Prop. C3 grass, scaled w/ ther types to sum to 1
ggarrange(
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = percC3grass_scaled_CONUS)) + 
...
widths = c(1, .5)
 ...
 ## step 8: Prop. C4 grass, scaled w/ ther types to sum to 1
ggarrange(
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = percC4grass_scaled_CONUS)) + 
...
widths = c(1, .5)
 ...
 ## step 9: Prop. Forb, scaled w/ ther types to sum to 1
ggarrange(
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = percForb_scaled_CONUS)) + 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = absBLTree_F)) + 
  ggtitle("Absolute BL tree cover, from model fit in Forest \n(Prior to scaling by predicted forest prob.!)"), 
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  facet_wrap(~prob_Forest_b) + 
  geom_density(aes(x = absBLTree_F))+ 
  geom_vline(aes(xintercept = .1), lty = 2) +
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = absC3grass_CONUS)) + 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = absC4grass_CONUS)) + 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = absForb_CONUS)) + 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = absNLTree_CONUS)) + 
...
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
  geom_point(aes(x = x, y = y, col = absBLTree_CONUS)) + 
  ggtitle("Absolute BL tree cover, from model fit in Forest \n(After scaling by predicted forest prob.!)"), 
preds_quantile %>% 
  drop_na(prob_Forest_b) %>% 
  ggplot() + 
...
# NL trees
# predictions for absolute needle leaved tree cover
decilePredNames_NLTree<- c(str_remove_all(c(str_split(names(coefficients(NLtree_F)), ":", simplify = TRUE))[-1], pattern = regex("[I\\(]")) %>% 
  str_remove(pattern = regex("[2^\\)].")) %>% 
  str_remove(pattern = regex("\\)"))) %>% 
  unique()
...
decilePredNames_BLTree<- c(str_remove_all(c(str_split(names(coefficients(BLtree_F)), ":", simplify = TRUE))[-1], pattern = regex("[I\\(]")) %>% 
  str_remove(pattern = regex("[2^\\)].")) %>% 
  str_remove(pattern = regex("\\)"))) %>% 
  unique()
...
decilePredNames_totalHerb <- c(str_remove_all(c(str_split(names(coefficients(totalHerb_F)), ":", simplify = TRUE))[-1], pattern = regex("[I\\(]")) %>% 
  str_remove(pattern = regex("[2^\\)].")) %>% 
  str_remove(pattern = regex("\\)")) ,
  str_remove_all(c(str_split(names(coefficients(totalHerb_GS)), ":", simplify = TRUE))[-1], pattern = regex("[I\\(]")) %>% 
  str_remove(pattern = regex("[2^\\)].")) %>% 
  str_remove(pattern = regex("\\)")) 
) %>% 
  unique()
...
decilePredNames_totalHerb_GS <- c(
  str_remove_all(c(str_split(names(coefficients(totalHerb_GS)), ":", simplify = TRUE))[-1], pattern = regex("[I\\(]")) %>% 
  str_remove(pattern = regex("[2^\\)].")) %>% 
  str_remove(pattern = regex("\\)")) 
) %>% 
...
preds_trees <- preds_quantile %>% 
  select(TotalTreeCover, absTotalTree_F, absTotalTree_GS,
         ...
         percNLTree_F, percNLTree_scaled_F, #percNLTree_scaled_GS, 
         NLTreeCover_abs, absNLTree_CONUS, #absNLTree_GS, 
         absNLTree_F,
         AngioTreeCover_prop, #percBLTree_GS, 
         percBLTree_F, percBLTree_scaled_F, #percBLTree_scaled_GS, 
         BLTreeCover_abs, absBLTree_CONUS,
         #absBLTree_GS, 
         absBLTree_F,
...
# for the entire CONUS using the F model
decilePredNames_NLTree_F <- c(
  str_remove_all(c(str_split(names(coefficients(NLtree_F)), ":", simplify = TRUE))[-1], pattern = regex("[I\\(]")) %>% 
  str_remove(pattern = regex("[2^\\)].")) %>% 
  str_remove(pattern = regex("\\)")) 
) %>% 
  unique()
decilePredNames_NLTree_F <- decilePredNames_NLTree_F[decilePredNames_NLTree_F != ""]
...
deciles_NLtree_absolute_CONUS <- predvars2deciles(df = preds_trees %>% select(NLTreeCover_abs, absNLTree_CONUS, unique(decilePredNames_NLTree_F, decilePredNames_totalTree_F)) %>% drop_na()
                                   , response_vars = c("NLTreeCover_abs", "absNLTree_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c(decilePredNames_NLTree_F), # for predictors, combine list of predictors for all models (total tree for F and F, and perc NL tree for F and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_NLtree_absolute_CONUS <- add_dotplot_inset(quantPlot_NLtree_absolute_CONUS, df = deciles_NLtree_absolute_CONUS, response = c("NLTreeCover_abs", "absNLTree_CONUS"), dfRaw = preds_trees %>% select(NLTreeCover_abs, absNLTree_CONUS, unique(decilePredNames_NLTree_F, decilePredNames_totalTree_F)) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)
...
deciles_BLtree_absolute_CONUS <- predvars2deciles(df = preds_trees %>% select(BLTreeCover_abs, absBLTree_CONUS, unique(decilePredNames_BLTree_F, decilePredNames_totalTree_F)) %>% drop_na()
                                   , response_vars = c("BLTreeCover_abs", "absBLTree_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c(unique(decilePredNames_BLTree_F, decilePredNames_totalTree_F)), # for predictors, combine list of predictors for all models (total tree for F and F, and perc BL tree for F and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_BLtree_absolute_CONUS <- add_dotplot_inset(quantPlot_BLtree_absolute_CONUS, df = deciles_BLtree_absolute_CONUS, response = c("BLTreeCover_abs", "absBLTree_CONUS"), dfRaw = preds_trees %>% select(BLTreeCover_abs, absBLTree_CONUS, unique(decilePredNames_BLTree_F, decilePredNames_totalTree_F)) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)
...
predsOverTime_long <- predsOverTime %>% 
  select(year,x, y, spatialID, uniqueID, absShrub_CONUS, absBareGround_CONUS, absTotalHerb_CONUS, absTotalTree_CONUS, absC3grass_CONUS:absNLTree_CONUS) %>% 
  pivot_longer(cols = c(absShrub_CONUS, absBareGround_CONUS, absTotalHerb_CONUS, absTotalTree_CONUS, absC3grass_CONUS:absNLTree_CONUS)) %>% 
