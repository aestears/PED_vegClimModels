---
title: "03B_troubleshooting_modelfit"
author: "Alice Stears"
...
params:
  ...
  response: AngioTreeCover_abs #TotalTreeCover
...
# calculate absolute NL tree cover and absolute BL tree cover
modDat$AngioTreeCover_abs <- modDat$AngioTreeCover_prop * modDat$TotalTreeCover
modDat$ConifTreeCover_abs <- modDat$ConifTreeCover_prop * modDat$TotalTreeCover
...
modDat[modDat$AngioTreeCover_abs == 0 & !is.na(modDat$AngioTreeCover_abs), "AngioTreeCover_abs"] <- 0.0001
modDat[modDat$ConifTreeCover_abs == 0 & !is.na(modDat$ConifTreeCover_abs), "ConifTreeCover_abs"] <- 0.0001
...
modDat[modDat$AngioTreeCover_abs == 1 & !is.na(modDat$AngioTreeCover_abs), "AngioTreeCover_abs"] <- 0.9999
modDat[modDat$ConifTreeCover_abs == 1 & !is.na(modDat$ConifTreeCover_abs), "ConifTreeCover_abs"] <-0.9999
```
...
"annWatDef_anom", "annWetDegDays_anom", "VPD_min_anom"  , "frostFreeDays_anom")
 ...
 c(
"tmean"             , "prcp"          , 
"prcp_dry"          , "prcpTempCorr"          ,"isothermality"    , "annWatDef" ,
...
<!-- ## If predicting total tree cover, restrict the data only to the locations where we have observations for total tree cover and the proportion of that total tree cover that is either needle-leaved or broad-leaved -->
<!-- ```{r} -->
...
```{r visualize response variable}
 modDat_1_s%>% 
  ...
  ggtitle( paste0("Histogram of ",response))
...
ggplot(scaleFigDat_3) + 
  ...
  ggtitle("The distribution of unscaled predictor variables")
...
ggplot(scaleFigDat_3) + 
  ...
  geom_histogram(aes(predValues_scaled), fill = "lightblue", col = "blue") +
  xlab ("predictor variable values") + 
  ggtitle("The distribution of scaled predictor variables")
```
...
plot2<-ggplot(data=modDat_1_sf %>%
                pivot_longer(cols=c(tmean:prcp)),
...
# ```{r}
# ## test subset the data to unimodal ecoregions
# modDat_1_s <- modDat_1_s %>% 
#   filter(NA_L1NAME %in% c("WESTERN CORDILLERA" , "ATLANTIC HIGHLANDS", "MARINE WEST COAST FOREST"))
# ```
...
prednames_new <-   c("tmean" , "I(log(prcp+1))",  
                     "I(log(prcp_dry+1))",  "prcpTempCorr",
                     "isothermality" , "I(log(annWatDef+2))"  , "sand" ,  "coarse" ,  "I(log(carbon+1))",  "AWHC",                    "tmin_anom",  "tmax_anom" , "t_warm_anom" ,  "prcp_wet_anom",  "precp_dry_anom" ,  "prcp_seasonality_anom",   "prcpTempCorr_anom" , "aboveFreezingMonth_anom", "isothermality_anom",  #"annWatDef_anom",
                      "annWetDegDays_anom",      "VPD_min_anom")
  #c(
# "tmin"             ,"tmax"        , "tmean"     , "I(log(prcp+1))"             ,"t_warm"          ,
# "t_cold"           ,"prcp_wet"    , "I(log(prcp_dry+1))"  , "prcp_seasonality" ,#"prcpTempCorr"    ,
# "abvFreezingMonth" ,"isothermality", "I(log(annWatDef+2))",  "I(log(annWetDegDays+2))"   , "VPD_mean"       , 
# "VPD_max"          ,"VPD_min"     , "VPD_max_95", "annWatDef_95"     ,"annWetDegDays_5" ,
# "frostFreeDays_5"  ,"frostFreeDays", "soilDepth",  "clay"            , "sand"           , 
# "coarse"           ,"I(log(carbon+1))"      , "AWHC"           
# )
...
if (removeAllAnoms) { ## if the model parameters indicate that we don't want any anomalies... 
 ...
 ## break predictors into climate, weather, and soils 
  prednames_clim <- prednames_new[which(prednames_new %in% c("tmin" , "tmax",  "tmean",
     "I(log(prcp+1))", "t_warm" , "t_cold" , "prcp_wet" , "I(log(prcp_dry+1))" , "prcp_seasonality", "prcpTempCorr", "abvFreezingMonth" , "isothermality" , "I(log(annWatDef+2))" , "annWetDegDays",  "VPD_mean",  "VPD_max",  "VPD_min" , "VPD_max_95", "annWatDef_95", "annWetDegDays_5",  "frostFreeDays_5", "frostFreeDays"))]

  prednames_weath <- prednames_new[which(prednames_new %in% c("tmean_anom", "tmin_anom",  "tmax_anom", 
  ...
  prednames_soils <- prednames_new[which(prednames_new %in% c( "soilDepth" , "clay", 
     "sand" , "coarse" ,  "I(log(carbon+1))", "AWHC"))]
...
Visualize all of the variables in the final list of possible predictors 
```{r fig.width = 12, fig.height =10}
as.data.frame(X) %>% 
  select(-"(Intercept)") %>% 
  pivot_longer(cols = any_of(colnames(X)), names_to = "variable", values_to = "values") %>% 
  ggplot() + 
...
if (run == TRUE) {
  ...
  lambda_halfSE <- best_lambda + ((lambda_1SE - best_lambda)/2)
          ...
          ## testing
     x <- quasibeta_family(link = "logit")
  pred_rawLASSO <- x[["linkinv"]](glmnet::predict.glmnet(object = fit$glmnet.fit, newx = X[,2:ncol(X)], s = best_lambda)  )
  pred_rawLASSO_1se <- x[["linkinv"]](glmnet::predict.glmnet(object = fit$glmnet.fit, newx = X[,2:ncol(X)], s = lambda_1SE)  )
  pred_rawLASSO_unreg <- x[["linkinv"]](glmnet::predict.glmnet(object = fit$glmnet.fit, newx = X[,2:ncol(X)], s = 0)  )
...
#      )
# lines(density(pred_rawLASSO), col = "red")  
# lines(density(pred_rawLASSO_1se), col = "orange")
# lines(density(pred_rawLASSO_unreg), col = "blue")
...
## fit using betareg
  fit_glm_bestLambda <- fit_glm_bestLambda_quasiBETA <- betareg(
      formula = f_glm_bestLambda, data = modDat_1_s, link = "logit"
  )
  # fit_glm_bestLambda_varDev <-  betareg(
  #     formula = as.formula(paste0(response, " ~ ", paste0(paste0(mat_glmnet_best, collapse = " + "), "| I(log(prcp + 1)) + prcp_wet")))
  #       
  #                      , data = modDat_1_s, link = "logit"
  # )
  # 
  # AIC(fit_glm_bestLambda, fit_glm_bestLambda_varDev)
    # glm(formula = f_glm_bestLambda,data = X_dataFrame#modDat_1_s
    #                                                         , family =quasibeta_family(link = "logit") )
  ...
  #fit_glm_bestLambda_BETAREG <- betareg(formula =  f_glm_bestLambda, data = X_dataFrame#modDat_1_s
    ...
    # by hand
  #fit_glm_bestLambda_BETAREG_byHand <- betareg(formula = TotalTreeCover ~ tmean + prcp + prcp_dry + prcpTempCorr + isothermality + 
   #  annWatDef + coarse + carbon + AWHC + prcpTempCorr_anom + 
   #  isothermality_anom + annWatDef_anom + I(tmean^2) + I(isothermality^2) + 
   #  I(annWatDef_anom^2) + I(AWHC^2) + isothermality_anom:annWatDef_anom + 
   #  prcpTempCorr:annWatDef_anom + prcpTempCorr_anom:annWatDef_anom + 
   #  tmean:annWatDef_anom + prcpTempCorr:isothermality + isothermality:prcpTempCorr_anom + 
   #  prcp:isothermality_anom + tmean:isothermality_anom + prcp:prcpTempCorr_anom + 
   #  prcp_dry:prcpTempCorr + tmean:prcp_dry + tmean:prcpTempCorr + 
   #  carbon:AWHC + coarse:AWHC + sand:AWHC + coarse:carbon + sand:carbon + 
   #  sand:coarse, 
   #  data = modDat_1_s, 
   #  link = "logit")
   # fit_glm_bestLambda_BETAREG_byHandSTAR <- betareg(formula = TotalTreeCover ~ tmean + prcp + prcp_dry + prcpTempCorr + isothermality + 
   #  annWatDef + coarse + carbon + AWHC + prcpTempCorr_anom + 
   #  isothermality_anom + annWatDef_anom + I(tmean^2) + I(isothermality^2) + 
   #  I(annWatDef_anom^2) + I(AWHC^2) + isothermality_anom*annWatDef_anom + 
   #  prcpTempCorr*annWatDef_anom + prcpTempCorr_anom*annWatDef_anom + 
  ...
  fit_glm_se <- betareg(#glm(
    formula = f_glm_1se, data = modDat_1_s
                    , #family =quasibeta_family(link = "logit"),
      link = "logit")
  ...
  fit_glm_halfse <- #glm(
    betareg(formula = f_glm_halfse, data = modDat_1_s
                        , #family =quasibeta_family(link = "logit") 
            link = "logit")
...
```
Try fitting a ZOIB (zero-inflated beta regression) 
```{r}
library("brms")
model_formula <- brms::bf(
  ConifTreeCover_abs ~ tmean + I(log(prcp + 1)) + prcpTempCorr + 
    isothermality + sand + coarse + I(log(carbon + 1)) + AWHC + 
    I(log(prcp_dry + 1)):I(log(annWatDef + 2)) + tmean:I(log(annWatDef + 
    2)) + I(log(prcp + 1)):I(log(prcp_dry + 1)) + I(log(prcp_dry + 
    1)):prcpTempCorr + prcpTempCorr:isothermality + tmean:isothermality + 
    tmean:prcpTempCorr + coarse:AWHC + I(log(carbon + 1)):AWHC + 
    sand:AWHC + sand:coarse,
  phi ~ 1, 
  zi ~ tmean + I(log(prcp + 1)) + prcpTempCorr + 
    isothermality + sand + coarse + I(log(carbon + 1)) + AWHC + 
    I(log(prcp_dry + 1)):I(log(annWatDef + 2)) + tmean:I(log(annWatDef + 
    2)) + I(log(prcp + 1)):I(log(prcp_dry + 1)) + I(log(prcp_dry + 
    1)):prcpTempCorr + prcpTempCorr:isothermality + tmean:isothermality + 
...
model_zi_beta <- brm(formula = model_formula, 
                     data = modDat_1_s,
...
optimal_pred <- betareg::predict(fit_glm_bestLambda, newdata = modDat_1_s, type = "response")
  # plot(density(optimal_pred_OLD)) 
  # lines(density(optimal_pred_NEW), col = "red")

#optimal_pred_varDev <- betareg::predict(fit_glm_bestLambda_varDev, newdata = modDat_1_s, type = "response")
  
 optimal_pred_ByHand <- exp(betareg::predict(fit_glm_bestLambda, newdata = modDat_1_s))/
   (1 +  exp(betareg::predict(fit_glm_bestLambda, newdata = modDat_1_s)))
  optimal_pred_1se <-  betareg::predict(fit_glm_se, newdata = modDat_1_s, type = "response")
  optimal_pred_halfse <- betareg::predict(fit_glm_halfse, newdata = modDat_1_s, type = "response")
  
    null_fit <- betareg(#glm(#data = data.frame("y" = y, X[,paste0(prednames, "_s")]), 
      formula = y ~ 1, #data = modDat_1_s, 
      #family = quasibeta_family(link = "logit")
      link = "logit"
      #formula = y ~ 1, family = stats::Gamma(link = "log")
      )
  null_pred <- betareg::predict(null_fit, newdata = as.data.frame(X), type = "response"
                       )
...
optimal_pred_betareg <- betareg:::predict.betareg(object = fit_glm_bestLambda, newdata = modDat_1_s, type = "response")
...
x <- quasibeta_family()
  ...
  pred_rawLASSO <- x[["linkinv"]](glmnet::predict.glmnet(object = fit$glmnet.fit, newx = X[,2:ncol(X)], s = best_lambda)  )
  pred_rawLASSO_link <- glmnet::predict.glmnet(object = fit$glmnet.fit, newx = X[,2:ncol(X)], s = best_lambda, type = "link")
  # predictions from unregularized LASSO (all possible predictors)
  
  pred_rawLASSO_ALL <- x[["linkinv"]](glmnet::predict.glmnet(object = fit$glmnet.fit, newx = X[,2:ncol(X)], s = fit$lambda[200])  )
  pred_rawLASSO_link_ALL <- glmnet::predict.glmnet(object = fit$glmnet.fit, newx = X[,2:ncol(X)], s = fit$lambda[200], type = "link")
  # predictions by hand 
  predVector <- fit$glmnet.fit$beta[,200#fit$index["min",]
                                    ]
  ...
  # plot(density(y), col = "black", xlim = c(-2, 2), ylim = c(0,3),
  #      main = "predictions from LASSO model (prior to stability selection, trim anoms, etc.) \ngreen = preds. on the logit-link scale, \nred = preds. on the response scale; black = observations \nsolid line = best lambda LASSO, dashed line = unregularized LASSO", 
  #      )
  # #lines(density(pred_rawLASSO_link), col = "green")
  # #lines(density(pred_rawLASSO), col = "red") 
  # #lines(density(pred_rawLASSO_link_ALL), col = "green", lty = 2)
  # #lines(density(pred_rawLASSO_ALL), col = "red", lty = 2)
  # lines(density(optimal_pred_betareg_byHand), col = "purple")
  # lines(density(optimal_pred_betareg_byHandSTAR), col = "blue")
  # 
  #   
...
plot(density(fullModOut$modelPredictions$obs), 
     ...
     main = c("black = observed values \n red = predictions from 'best lambda' model\n orange = predictions for '1/2se' lambda model \n green = predictions from '1se' lambda model 
              blue = preds. from LASSO (best lambda) \npurple = preds. from unregularized LASSO"))
lines(density(fullModOut$modelPredictions$pred_opt), col = "red")
#lines(density(optimal_pred_varDev), col = "brown")
...
lines(density(pred_rawLASSO), col = "blue")  
#lines(density(pred_rawLASSO_1se), col = "")
lines(density(pred_rawLASSO_unreg), col = "purple")
...
prednames_fig <- str_replace(prednames_fig, "log\\(", "")
prednames_fig <- str_replace(prednames_fig, " \\+ 1\\)\\)", "")
prednames_fig <- str_replace(prednames_fig, " \\+ 2\\)\\)", "")
...
prednames_fig_1se <- str_replace(prednames_fig_1se, "log\\(", "")
prednames_fig_1se <- str_replace(prednames_fig_1se, " \\+ 1\\)\\)", "")
prednames_fig_1se <- str_replace(prednames_fig_1se, " \\+ 2\\)\\)", "")
...
prednames_fig_halfse <- str_replace(prednames_fig_halfse, "log\\(", "")
prednames_fig_halfse <- str_replace(prednames_fig_halfse, " \\+ 1\\)\\)", "")
prednames_fig_halfse <- str_replace(prednames_fig_halfse, " \\+ 2\\)\\)", "")
...
pred_glm1 <- predict_by_response(fit_glm_bestLambda, modDat_1_s#newdata_DF#X[,2:ncol(X)]
                                 )
...
pred_glm1_1se <- predict_by_response(mod_secondBest, modDat_1_s)#newdata_DF)
# 
# # add back in true y values
...
geom_spatraster(data = pred_glm1_RAST, aes(fill = resid), maxcell = 50000000) + 
  geom_sf(data=cropped_states_2 ,fill=NA ) +
  geom_sf(data = badResids_high, col = "blue", pch = 20) +
  geom_sf(data = badResids_low, col = "red", pch = 20) +
...
Interlude to determine where the predictions at the two 'peaks' of the density distribution occur in space (and time?) 
```{r, eval = FALSE}
## identify the values of the peaks
densOfPreds <- (density(pred_glm1$TotalTreeCover_pred))
# function from stack overflow for finding local maxima
...
densOfPreds_max <- densOfPreds$x[localMaxima(densOfPreds$y)]

ggplot(pred_glm1) + 
  geom_density(aes(TotalTreeCover_pred)) + 
  geom_vline(aes(xintercept = densOfPreds_max[1]), col = "red")+ 
  #geom_vline(aes(xintercept = densOfPreds_max[2]), col = "red")+ 
  #geom_vline(aes(xintercept = densOfPreds_max[3]), col = "red") + 
  ggtitle("Locations of local maxima in the density of model predictions")

## now, round those values and identify model predictions that are close to those values
pred_glm1_sf2 <- pred_glm1 %>% 
  st_as_sf(coords = c("x", "y"), crs = crs("EPSG:4326")) %>% 
  st_transform(crs(test_rast))
pred_glm1_sf2$Peak1 <- FALSE
pred_glm1_sf2 <- pred_glm1_sf2 %>% 
  mutate(Peak1 = replace(Peak1, (TotalTreeCover_pred < densOfPreds_max[1] + .01) & (TotalTreeCover_pred > densOfPreds_max[1] - .01), TRUE))
#pred_glm1_sf[round(pred_glm1_sf$TotalTreeCover_pred, -1) == round(densOfPreds_max[1],-1), "Peak1"] <- TRUE
pred_glm1_sf2$Peak2 <- FALSE
#pred_glm1_sf2[round(pred_glm1_sf2$TotalTreeCover_pred, -1) == round(densOfPreds_max[2],-1), "Peak2"] <- TRUE
# pred_glm1_sf2 <- pred_glm1_sf2 %>% 
#   mutate(Peak2 = replace(Peak2, (TotalTreeCover_pred < densOfPreds_max[2] + .01) & (TotalTreeCover_pred > densOfPreds_max[2] - .01), TRUE))
## make a map showing the location of predictions in these 'peaks'
ggplot() +
geom_spatraster(data = pred_glm1_RAST, aes(fill = .data[[paste0(response,"_pred")]]), maxcell = 50000000) + 
 geom_sf(data=cropped_states_2 ,fill=NA ) +
 geom_sf(data = pred_glm1_sf2[pred_glm1_sf2$Peak1 == TRUE,], aes(), col = "purple", pch = 20) + 
 geom_sf(data = pred_glm1_sf2[pred_glm1_sf2$Peak2 == TRUE,], aes(), col = "orange", pch = 20) + 
labs(title = paste0("Location of model predictions that are in the bimodal 
                    peaks of the distribution of predictions; 
                    This shows predictions that are +/- .01 from the 
                    rounded value of the peak"),
     subtitle =  paste0("purple = values near peak 1 (",round(densOfPreds_max[1],2)#,"); 
     #orange = values near peak 2 (",round(densOfPreds_max[2],2),")"
     )
     )  +
  scale_fill_gradient2(low = "brown",
                       mid = "wheat",
...
pred_glm1_sf2 %>% 
  ggplot() + 
  geom_point(aes(x = Year, y = jitter(as.numeric(Peak1))), col = "purple") + 
  geom_smooth(aes(x = Year, y =jitter(as.numeric(Peak1))), col = "magenta") + 
  ggtitle("Are predictions in a given year near the first peak? (1 = yes, 0 = no)",
          subtitle = paste0("value of peak = ", round(densOfPreds_max[1],2)))
# pred_glm1_sf2 %>% 
#   ggplot() + 
#   geom_point(aes(x = Year, y = jitter(as.numeric(Peak2))), col = "orange") + 
#   geom_smooth(aes(x = Year, y =jitter(as.numeric(Peak2))), col = "darkorange") + 
#   ggtitle("Are predictions in a given year near the second peak? (1 = yes, 0 = no)",
#           subtitle = paste0("value of peak = ", round(densOfPreds_max[2],2)))


pred_glm1_sf2 %>% 
  ggplot() + 
  geom_point(aes(x = Year, y = TotalTreeCover)) + 
  geom_smooth(aes(x = Year, y = TotalTreeCover)) + 
  ggtitle("Observed total tree cover values by year")
...
modDat_1_s %>% 
  ...
  ggtitle("Number of total tree observations by year")
...
## what are the values of the predictors for locations near the peaks? 
# names of model terms
comparisonFigureNames <- c(  
  
   "tmean"                          , "log_prcp_dry"  ,                    
 "sand"                             , "coarse"                                     ,"log_carbon"    ,                    
 "AWHC"                             , "log_prcp_dry:log_annWatDef" ,"tmean:log_annWatDef" ,              
 "log_prcp_dry:isothermality", "tmean:isothermality"                       , "coarse:AWHC"                 ,              
 "log_carbon:AWHC"          , "sand:AWHC"          ,  "log_prcp:log_prcp_dry"                    
...
# "tmean"               , "log_prcp"                     ,
# "isothermality"                     , "sand"                , "coarse"                               ,
# "log_carbon"                , "AWHC"                , "log_prcp:log_prcp_dry",
# "isothermality:log_prcp_dry", "tmean:isothermality" , "log_carbon:AWHC"              ,
# "sand:coarse"       )   
pred_glm1_sf3 <- pred_glm1_sf2 %>% 
  mutate(#"log_prcp" = log(prcp + 1), 
          "log_prcp_dry" = log(prcp_dry+1),
         "log_carbon" = log(carbon +1),
         "log_prcp_dry:log_annWatDef" = log(prcp_dry+1)*log(annWatDef + 2),
         "tmean:log_annWatDef" = tmean*log(annWatDef + 2) ,   
         "log_prcp_dry:isothermality" =  log(prcp_dry+1)*isothermality,
         "log_prcp:log_prcp_dry" = log(prcp + 1) *log(prcp_dry + 1),
         #"isothermality:log_prcp_dry" = isothermality*log(prcp_dry + 1),
         "tmean:isothermality" = tmean*isothermality,
          "coarse:AWHC" =  coarse*AWHC,
         "log_carbon:AWHC" = log(carbon + 1)*AWHC,
          "sand:AWHC"=  sand*AWHC,
         "prcpTempCorr:tmean"  = prcpTempCorr*tmean,
         "prcpTempCorr:isothermality" = prcpTempCorr:isothermality
         #"sand:coarse" = sand*coarse
         ) %>% 
  pivot_longer(cols = any_of(comparisonFigureNames), names_to = "PredNames", values_to = "PredValues")
  ggplot() + 
    facet_wrap(~PredNames, scales = "free") + 
  geom_density(data = pred_glm1_sf3, aes(PredValues)) + 
  geom_density(data = pred_glm1_sf3[pred_glm1_sf3$Peak1 == TRUE,], aes(PredValues), col = "purple") + 
  #geom_density(data = pred_glm1_sf3[pred_glm1_sf3$Peak2 == TRUE,], aes(PredValues), col = "orange") + 
    ggtitle("Comparing the predictor values for points in the two different peaks (purple = peak 1, orange = peak 2)")
...
mat <- coef_glm_bestLambda
mat2 <- mat[(mat[,1] != 0),]
mat2 <- mat2[which(mat2$coefficientName != "(phi)"),]
f_cv <- as.formula(paste0(response, " ~ ", paste0(mat2$coefficientName[2:nrow(mat2)], collapse = " + ")))
...
fit_i <- betareg(data = trainDat_all, 
                   formula =f_cv,
    ,
    link = "logit"
               #family =  quasibeta_family(link = "logit")
  ...
  null_pred <- predict(fit_glm_bestLambda, newdata = testDat_all, type = "response") 
...
# now, loop through so with each iteration, a different ecoregion is held out
 for(i_eco in sort(unique(modDat_1_s$NA_L2NAME))){
  ...
  ## just try a regular glm w/ the components from the global model
  fit_i <- betareg(data = trainDat_all, formula = f_cv,
    ,
    link = "logit"
               #family =  quasibeta_family(link = "logit")
    )
  ...
  null_pred <- predict(mod_secondBest, newdata = testDat_all, type = "response") 
