...
```{r}
modDat_1_s <- modDat_1_s %>% 
  mutate(TotalTreeCover = TotalTreeCover * 10)
# because the beta glm does not allow values of 1, make the highest values very slightly less than 1
modDat_1_s[modDat_1_s$TotalTreeCover == 1, "TotalTreeCover"] <- 0.9999
```
...
if(ecoregion == "noTrees" & response != "TotalTreeCover") {
...
} else if(ecoregion == "noTrees" & response == "TotalTreeCover") {
   # 
   # modDat_1_s[modDat_1_s$NA_L2NAME %in% c("EVERGLADES", "SOUTHEASTERN USA PLAINS", "TEXAS-LOUISIANA COASTAL PLAIN", "MISSISSIPPI ALLUVIAL AND SOUTHEAST USA COASTAL PLAINS", "TAMAULIPAS-TEXAS SEMIARID PLAIN"), "NA_L2NAME"] <- "SOUTHEASTERN USA PLAINS AND EVERGLADES AND LOUISIANA PLAIN"
   # modDat_1_sf[modDat_1_sf$NA_L2NAME %in% c("EVERGLADES", "SOUTHEASTERN USA PLAINS", "TEXAS-LOUISIANA COASTAL PLAIN", "MISSISSIPPI ALLUVIAL AND SOUTHEAST USA COASTAL PLAINS", "TAMAULIPAS-TEXAS SEMIARID PLAIN"), "NA_L2NAME"] <- "SOUTHEASTERN USA PLAINS AND EVERGLADES AND LOUISIANA PLAIN"
   # 
...
if (run == TRUE) {
  ...
  relaxParam <- ifelse(response == "ShrubCover", yes = TRUE, no = 
                         ifelse(c(response == "TotalTreeCover" & ecoregion == "noTrees"), yes = TRUE, no = FALSE))
  # Fit model -----------------------------------------------
...
```{r}
# filter to only get locations that are model-predicted to be unforested
climDatPred_noForest <- climDatPred %>% 
...
climDatPred_noForest_rast <- climDatPred_noForest %>%
...
ggplot() +
  ...
  #         ,
  #         aes(fill=as.factor(NA_L2NAME)),linewidth=0.5,alpha=0.5, size = 1) +
  geom_spatraster(data=climDatPred_noForest_rast_mask , maxcell = 50000000) + 
  xlim(c(-1951750, 2229250)) +
  geom_sf(data=cropped_states2,fill='NA')  +
  scale_fill_viridis_c( na.value = "white") +
  labs(title = "Model-predicted unforested locations, with same mask Martin used for RAP data")
```
...
notForest_rast_mask <- nonForest_rast %>% 
  terra::mask(mask = climDatPred_noForest_rast %>% crop(y = nonForest_rast))
...
climDatPred_noForest_rast_mask <- climDatPred_noForest_rast %>% 
...
# hist(modDat_1_s[,response], main = paste0("Histogram of ",response),
#      xlab = paste0(response), breaks = 50)
# hist(pred_glm1[,response], breaks = 50,  col = "blue")
