...
## get RAP unforested cover dataset from Martin
  #
  #Data_processed/RAP_v3_fracNotForest_lt3_mask-lcmap50_2019-2023_1000m.tif
nonForest_rast <- terra::rast("../../../Data_processed/RAP_v3_fracNotForest_lt5_mask-lcmap50_2019-2023_1000m.tif") %>% 
...
if(response == "TotalTreeCover_transformed") {
  ...
  # interpolate second-best lambda predictions
  pred_glm1_1se$TotalTreeCover_transformed <- pred_glm1$TotalTreeCover_transformed/10
...
relativeResid <- (pred_glm1_RAST$TotalTreeCover_transformed -  pred_glm1_RAST$TotalTreeCover_transformed_pred)/pred_glm1_RAST$TotalTreeCover_transformed
# calculate relative residuals

# make figures
map <- ggplot() +
geom_spatraster(data = relativeResid, aes(fill = TotalTreeCover_transformed), maxcell = 50000000) + 
  geom_sf(data=cropped_states_2 ,fill=NA ) +
  #geom_sf(data = badResids_high, col = "blue") +
  #geom_sf(data = badResids_low, col = "red") +
...
hist <- ggplot(data.frame(values(relativeResid))) + 
  geom_histogram(aes(TotalTreeCover_transformed), fill = "lightgrey", col = "darkgrey") + 
  geom_text(aes(x = min(TotalTreeCover_transformed)*.9, y = 1500, label = paste0("min = ", round(min(TotalTreeCover_transformed),2)))) +
  geom_text(aes(x = max(TotalTreeCover_transformed)*.9, y = 1500, label = paste0("max = ", round(max(TotalTreeCover_transformed),2)))) + 
  geom_vline(aes(xintercept = mean(TotalTreeCover_transformed)))
...
# calculate actual residuals
relativeResid <- (pred_glm1_1se_RAST$TotalTreeCover_transformed -  pred_glm1_1se_RAST$TotalTreeCover_transformed_pred)/pred_glm1_1se_RAST$TotalTreeCover_transformed
# calculate relative residuals
...
hist <- ggplot(data.frame(values(relativeResid))) + 
  ...
  geom_text(aes(x = min(TotalTreeCover_transformed)*.9, y = 1500, label = paste0("min = ", round(min(TotalTreeCover_transformed),2)))) +
  geom_text(aes(x = max(TotalTreeCover_transformed)*.9, y = 1500, label = paste0("max = ", round(max(TotalTreeCover_transformed),2)))) + 
  geom_vline(aes(xintercept = mean(TotalTreeCover_transformed, na.rm = TRUE)))
...
climDatPred_ecoregion_glm1 <- climDatPred_ecoregion_glm1 %>% 
  mutate(TotalTreeCover_transformed_pred = approxfun(TotalTreeCover_transformed_pred))
