...
```{r}
  ...
  # get a text string of possible interactions 
  combos <- data.frame(gtools::permutations(n = length(prednames),
                                            r = 2,
                                            v = prednames))
  ...
  # make a text string of the moel formula w/ interactions included
  ...
  # specify folds for each observation to use in leave-one-year-out cross-validation
  my_folds <- as.numeric(as.factor(modDat_1$NA_L2NAME))
  ...
  optimal_pred <- predict(fit,newx=X_test,s="lambda.min")
  ...
  null_pred <- predict(null_fit, newdata = as.data.frame(X_test))
...
# make model matrix (which includes transformations and interactions)
  ...
  f <- as.formula(modWithInteractions)
   ...
   # get model matrices that include interaction terms 
  X_all <- model.matrix(f, modDat_1)
 ...
 for(i_eco in sort(unique(modDat_1$NA_L2NAME))){
  ...
  X_train <- as.matrix(X_all[train,])
  X_test <- as.matrix(X_all[test,])
