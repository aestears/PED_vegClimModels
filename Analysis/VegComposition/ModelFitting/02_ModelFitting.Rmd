...
"VPD_min_anom"           , "VPD_max_95_anom"    ,"annWatDef_95_anom"    ,"annWetDegDays_5_anom",   
...
"annWatDef_95_anom"   , "annWetDegDays_5_anom", "frostFreeDays_5_anom"   
    )
}
...
With the full dataset (full response variables) -- try using bamlss package to do a beta regression (which allows us to use LASSO later on, I think...)
```{r}
# try using bamlss
nullMod_bamlss <- bamlss(data = modDat_1, formula = modDat_1[,response] ~ 1, family = "beta", link = "logit")
plot(nullMod_bamlss)

# results from the betareg function are the same! (or close enough)
# nullMod_beta <- betareg(formula = modDat_1[,response] ~ 1,
#                               link = c("logit"), link.phi = NULL,
#                                type = c("ML"))

summary(nullMod_bamlss)
# summary(nullMod_beta)
```
