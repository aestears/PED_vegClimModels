...
```{r}
if (byRegion == FALSE) {
  var_transformed <- map(mods_glm_transf, function(x) x$var_transformed)
var_transformed
} else {
  # for western forests
...
var_transformed_df <- purrr::flatten_dfr(var_transformed) #%>%
...
modWithInteractions <- paste(str_flatten(var_transformed_df, collapse = " + "), "+", int_string) %>% str_remove("stats::")
