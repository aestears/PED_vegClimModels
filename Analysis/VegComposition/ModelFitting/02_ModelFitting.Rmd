...
```{r fit global model, echo = FALSE}
 ...
 ## break predictors into climate, weather, and soils 
  ...
  transform_string <- str_flatten(c(#paste0("I(log(",prednames_clim,"))"), 
                                    #paste0("I(log(",prednames_weath,"))"),
                                    #paste0("I(log(",prednames_soils,"))"),
  ...
  # glmnet:::coef.cv.glmnet(fit, s = "lambda.min")
  #  
  ## predict on the test data
  # lasso model predictions with the optimal lambda
  optimal_pred <-  glmnet:::predict.cv.glmnet(fit,newx=X,s="lambda.min", type = "response")
...
legend(x = 19, y = 150, legend = c("Obs. values", "Preds - best mod.", "Preds - null mod."), 
       col = c("black", "red", "blue"), pch = 1, cex = .7)
...
```{r code from Gregor for cross-validation... not used currently, include=FALSE}
# ## code from Gregor
...
```{r Residual maps2, fig.height = 7, fig.width = 9, }
# rasterize data
...
pred_glm1 %>%
  select(prednames_fig, response_vars) %>%
...
```{r Percentile_plot, fig.height= 12, fig.width = 12}
...
```{r glm_deciles_filtered, fig.height= 14, fig.width = 14, message = FALSE}
...
## Use terms from global model to re-fit and predict on different held out regions (coming later...)
...
# 
# X_cv <- model.matrix(object = f, data = modDat_1)
# # get response variable
# y_cv <- as.matrix(modDat_1[,response])
#   
# 
...
#   train <- which(modDat_1$NA_L2NAME!=test_eco) # data for all ecoregions that aren't 'i_eco'
#   test <- which(modDat_1$NA_L2NAME==test_eco) # data for the ecoregion that is 'i_eco'
#   
...
#   fit_i <- glm(data = trainDat_all, formula = f, family = stats::Gamma(link = "log"))
# 
#     coef(fit_i)
# 
#   # lasso model predictions with the optimal lambda
#   optimal_pred <- predict(fit_i, newdata= testDat_all, scale = "response"
#                           )
#   # null model and predictions
#   # the null model is predictions w/ the global model 
#   #null_fit <- glm(data = as.data.frame(X_train), formula = y_train ~ 1, family = stats::Gamma(link = "log"))
#   
#   null_pred <- glmnet:::predict.cv.glmnet(fit, newx = as.matrix(X[test,]), scale = "response", s=min(fit$lambda))
# 
#   # save data
...
## plot residuals for each of the models fit to held-out ecoregions 
These models were fit to eight ecoregions, and then predict on one heldout ecoregion

Observations across the temporal range of the dataset
```{r Residual maps4, fig.height = 7, fig.width = 9, }
# for (i in 1:length(unique(modDat_1$NA_L2NAME))) {
...
# geom_spatraster(data = plotObs) + 
#   geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)),fill=NA ) +
# labs(title = paste0("Residuals (obs. - pred.) for predictions of ", holdoutRegion, " from a model fit to other ecoregions"),
...
# 
# ggarrange(
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[1]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[2]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[3]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[4]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[5]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[6]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[7]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[8]),
#   get(paste0("residPlot_", unique(modDat_1$NA_L2NAME))[9]), 
