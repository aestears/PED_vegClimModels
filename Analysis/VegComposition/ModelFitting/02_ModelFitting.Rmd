...
```{r fit global model, echo = FALSE}
 ...
 ## break predictors into climate, weather, and soils 
  ...
  model.frame(formula = f, data = modDat_1_s)
  X <- model.matrix(terms(f), modDat_1_s)
  ...
  glmnet:::coef.cv.glmnet(fit, s = best_lambda #"lambda.min"
                          )
...
pred_glm1 <- pred_glm1 %>% 
  cbind(modDat_1[,c("Long", "Lat", "Year")])
...
```{r glm_deciles_filtered, fig.height= 18, fig.width = 18, message = FALSE}
...
```{r fig.height = 18, fig.width = 18, eval = TRUE}
...
#
  ...
  null_pred <- glmnet:::predict.cv.glmnet(fit, newx = X_test_glob[,2:ncol(X_test_glob)], scale = "response", s=min(fit$lambda))
