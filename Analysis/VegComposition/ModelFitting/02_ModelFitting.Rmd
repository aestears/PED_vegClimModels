...
-- use level 2 ecoregion for cross-validation to tune lambda in the LASSO model 
```{r fit global model, echo = FALSE}
 ...
 ## break predictors into climate, weather, and soils 
  ...
  # hold out one ecoregion (arbitrarily chose one, "Western Sierra Madre Piedmont" )
  ...
  optimal_pred <- predict(fit,newx=X_test,s="lambda.min", type = "response")
  ...
  null_fit <- glm(data = as.data.frame(X_train)[,prednames], formula = y_train ~ 1, family = stats::Gamma(link = "log"))
  null_pred <- predict(null_fit, newdata = as.data.frame(X_test), type = "response"
                       )
  ...
  fullModOut <- list(
    ...
    "modelPredictions" = data.frame(ecoRegion_holdout = rep(test_eco,length(y_test)),obs=y_test,
...
my_cors <- c(cor(optimal_pred[,1], y_test),
            cor(null_pred, y_test)
            )

# calculate mse between null and optimal model 
my_mse <- c(mean((obs_pred$pred_opt - y_test)^2) ,
            mean((obs_pred$pred_null - y_test)^2)#,
...
plot(y = fullModOut$modelPredictions$obs, x = X_test[,2], xlab = c("tmean"), ylab = (paste(response))) # observed values
points(y = fullModOut$modelPredictions$pred_opt, x = X_test[,2], col = "red") ## predictions w/ the CV model
points(y = fullModOut$modelPredictions$pred_null, x = X_test[,2], col = "blue") ## predictions with the null model
...
mat <- as.matrix(coef(fullModOut[1]$modelObject)) 
mat2 <- mat[mat[,1] > 0,]
...
globModTerms <- data.frame("Coefficient_Name" = names(mat2), 
                   "Value_From_LASSO_withCV" = unname(mat2)#, 
