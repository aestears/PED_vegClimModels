...
```{r}
mod_glmFinal <- readRDS("./models/glm_beta_model_CONUSwide_ShrubCover_n50000.RDS")

if (byRegion == TRUE) {
  mod_glmFinal_EF <- readRDS("./models/glm_beta_model_EasternForests_ShrubCover_n50000.RDS")
mod_glmFinal_WF <- readRDS("./models/glm_beta_model_WesternForests_ShrubCover_n50000.RDS")
mod_glmFinal_G <- readRDS("./models/glm_beta_model_GrassShrub_ShrubCover_n50000.RDS")
...
plotResid_rast_ALL <- pred_glm1_ALL %>% 
  ...
  terra::crop(ext(-2000000, 2500000, -2000000, 1200000))
# plot
residMap_allShrubs <- ggplot() + 
  geom_spatraster(data = plotResid_rast_ALL) + 
  ggtitle(paste0("Resids. from regional models of shrub cover")) + 
  scale_fill_viridis_c(option = "turbo", limits = c(-.55, .91), na.value = "white", 
                       ) #+ 
...
plotPreds_rast_ALL <- pred_glm1_ALL %>% 
         drop_na(ShrubCover_dec) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("Lon", "Lat")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
  terra::rasterize(y = test_rast, 
                   field = "ShrubCover_dec", 
                   fun = mean) %>% 
  terra::crop(ext(-2000000, 2500000, -2000000, 1200000))

predsMap_allShrubs <- ggplot() + 
  geom_spatraster(data = plotPreds_rast_ALL*100) + 
  ggtitle(paste0("Predictions from regional models of shrub cover")) + 
  scale_fill_viridis_c(option = "viridis", limits = c(0, 100), na.value = "white", 
                       ) #+ 1

predsMap_allShrubs / residMap_allShrubs
}
...
ggplot(pred_glm1_ALL)  + 
  geom_point(aes(ShrubCover_dec, ShrubCover_dec_pred))

  
df <- pred_glm1_deciles_ALL 
max <- df %>% 
    select(ShrubCover_dec, ShrubCover_dec_pred) %>% 
    max * 100
inset <- ggplot(df, aes(ShrubCover_dec*100, 
                        ShrubCover_dec_pred*100)) +
    geom_point(shape = 4, alpha = 0.5, size = 0.3) +
    geom_abline(slope = 1, size = 0.5) +
    labs(y = "Observed Shrub Cover (%) - deciles",
         x = "Predicted Shrub Cover (%) - deciles") +
...
df <- pred_glm1_deciles 
max <- df %>% 
    select(ShrubCover_dec, ShrubCover_dec_pred) %>% 
    max * 100
inset2 <- ggplot(df, aes(ShrubCover_dec*100, 
                        ShrubCover_dec_pred*100)) +
    geom_point(shape = 4, alpha = 0.5, size = 0.3) +
    geom_abline(slope = 1, size = 0.5) +
    labs(y = "Observed Shrub Cover (%) - deciles",
