...
* vpdmean, min, and max, VPD_max_95 (keep tmean)
* precip_wet (prcp)
* tmax, tcold, frostfreedays, frostfree days_5, abvfreezingmonth (tmin)
* annwatdef_95 (annwatdef)
...
* 'prcp' and 'carbon': cor =  .76; chose 'prcp' (opposite of recommendation by function, but organic carbon is really skewed)
* 'tmean' and 'isothermality': cor =  .83; chose 'tmean' (opposite of recommendation by function, but tmean is easier to calculate)
* 'clay' and 'sand': cor =  .77; chose 'sand'
* 't_warm' and 'tmin': cor =  .73; chose 'tmin'
...
* vpdmean, min, and max, and vpd_max_95 (keep tmean instead)
* precip_wet (prcp)
* tmax, tcold, frostfreedays, frostfree days_5, abvfreezingmonth (tmin)
* prcp_seaonality, annwatdef_95 (annwatdef)
* annwetdegdays_5 (annwetdegdays)
* soildepth (AWHC)
...
* 'tmean' and 'prcp': cor =  .75; chose 'prcp'
* 'annWatDef' and 'prcp_dry': cor =  .74; chose 'prcp_dry'
* 'sand' and 'clay': cor =  .72; chose 'sand' (opposite of function recommendation, but the distribtuion of sand is less skewed than clay)
...
(corrPlot <- 
   datGrassShrub %>% 
   dplyr::select(grassShrubClimVars, #1-#8
      #"AWHC"
 # anomaly variables
    #tmean_anom ,
     tmin_anom , #9
   ...
   t_cold_anom , #13
      #prcp_wet_anom , #14
      precp_dry_anom , #15
    prcp_seasonality_anom , #16
     prcpTempCorr_anom , #17
      aboveFreezingMonth_anom , #18 
    isothermality_anom , #19
       annWatDef_anom , #20
     annWetDegDays_anom , #21
      #VPD_mean_anom , #22
      VPD_min_anom , #23
      #VPD_max_anom , #24
     VPD_max_95_anom , #25
      annWatDef_95_anom , #26
      annWetDegDays_5_anom , #27
    frostFreeDays_5_anom , #28
      #frostFreeDays_anom #29
   ) %>% 
    #slice_sample(n = 5e3) %>% 
   cor()  %>% 
   caret::findCorrelation(cutoff = .7, verbose = TRUE, names = TRUE, exact = TRUE)) #%>% 
...
corrPlotNames <- names(corrPlot$data)
kable(data.frame("Climate_And_Soils_Variables" = c(corrPlotNames[1:8],rep_len(NA,length.out = 6)), 
                 "Weather_Anomaly_Variables" = corrPlotNames[12:25]), format = 'html') %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
...
corrPlotNames <- names(corrPlot$data)
kable(data.frame("Climate_And_Soils_Variables" = c(corrPlotNames[1:11],rep_len(NA,length.out = 3)), 
                 "Weather_Anomaly_Variables" = corrPlotNames[12:25]), format = 'html') %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
