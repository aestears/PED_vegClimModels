...
```{r}
dat %>% 
  select(tmean_meanAnnAvg_3yrAnom:durationFrostFreeDays_meanAnnAvg_3yrAnom, Year, x, y) %>% 
...
(corrPlot <- 
   datCONUS2 %>% 
   dplyr::select(conusClimVars2, #1-#11
   ...
   t_cold_anom , #16
    ...
    isothermality_anom , #22
     ...
     annWetDegDays_anom , #24
      ...
      VPD_max_anom , 
      VPD_max_95_anom , #26
       annWatDef_95_anom , #27
       annWetDegDays_5_anom , #28
     frostFreeDays_5_anom, #29
      frostFreeDays_anom #30
   ) %>% 
    #slice_sample(n = 5e3) %>% 
   cor()  %>% 
   caret::findCorrelation(cutoff = .8, verbose = TRUE, names = TRUE, exact = TRUE)) #%>% 
...
Compare VPD_mean_anom  and tmean_anom with corr  0.854; chose tmean_anom
Compare VPD_max_anom  and t_warm_anom with corr  0.848; chose t_warm_anom
Compare tmean_anom  and tmin_anom with corr  0.866; chose tmin_anom
Compare frostFreeDays_anom and aboveFreezingMonth_anom with corr  0.84; chose aboveFreezingMonth_anom
...
corrPlotNames_conus2 <- names(corrPlot$data)
kable(data.frame("Climate_And_Soils_Variables" = c(corrPlotNames_conus2[1:11],rep_len(NA,length.out = 7)), 
                 "Weather_Anomaly_Variables" = corrPlotNames_conus2[12:length(corrPlotNames_conus2)]), format = 'html') %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
...
kable(data.frame("Climate_And_Soils_Variables" = c(corrPlotNames_grassShrub[1:9],rep_len(NA,length.out = 3)), 
                 "Weather_Anomaly_Variables" = corrPlotNames_grassShrub[10:length(corrPlotNames_grassShrub)]), format = 'html') %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
...
kable(data.frame("Climate_And_Soils_Variables" = c(corrPlotNamesForest[1:11],rep_len(NA,length.out = 2)), 
