...
zeroCutoff_bestLambda <- quantile(pred_glm1_w2w$TotalTreeCover_binom_pred %>% na.omit(), probs = 0.3133397) 
# plot the ecdf
...
abline(h = 0.3133397, lty = 3)
abline(v = zeroCutoff_bestLambda, lty = 3)
```
...
abline(h = 0.3133397, lty = 2)
abline(v = zeroCutoff_1seLambda, lty = 3)
```
...
approxfun_bestLambda <- approxfun(x = c(zeroCutoff_bestLambda,  best_threshold_bestLambda, 1), # values we're interpolating *from*
...
approxfun_secondBestLambda <- approxfun(x = c(zeroCutoff_1seLambda,  best_threshold_secondBest, 1), # values we're interpolating *from*
          y =c(0, 0.1, 1), # what we're interpolation to (desired values at the end)
          method = "linear") 
...
pred_glm1_test <- pred_glm1_test %>% 
  ...
  mutate(TotalTreeCover_interp = replace(TotalTreeCover_interp, list = TotalTreeCover_binom_pred < zeroCutoff_bestLambda, 0))
...
pred_glm1_1se_test <- pred_glm1_1se_test %>% 
  ...
  mutate(TotalTreeCover_interp = replace(TotalTreeCover_interp, list = TotalTreeCover_binom_pred < zeroCutoff_1seLambda, 0))
...
predData <- predData %>% 
  ...
  mutate(treeProb_bestLambda_continuous = replace(treeProb_bestLambda_continuous, list = treeProb_bestLambda < zeroCutoff_bestLambda, 0)) %>% # replace NAs at the 'bottom' of the interpolation scale with 0s
