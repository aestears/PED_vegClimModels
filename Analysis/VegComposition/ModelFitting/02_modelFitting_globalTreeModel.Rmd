...
modDat_1_rast <- modDat_1 %>%  
  st_as_sf(coords = c("x", "y"), crs = crs(vegDat)) %>% 
  terra::vect() %>% 
...
notForest <- notForest %>% 
  terra::project(crs(modDat_1_rast))
# make sure crs matches
crs(notForest) == crs(modDat_1_rast)
# crop modDat_1_rast to match the extent of the notForest data
modDat_1_rast <- modDat_1_rast %>% 
  terra::crop(ext(notForest))

## where are the locations that are masked in the layer from Martin, but un-masked in my data? (i.e. are there locations that I should not include in my model fitting dataset?)
# round vaues and change 0s to 1, so that every location w/ data has a value of '1'
notForest_TEST <- notForest %>% 
  round() %>% 
  subst(from = c(0), to = c(1)) %>% 
  subst(from = c(NA), to = c(0))
# change NAs to -9999
modDat_1_rast_TEST <- 
  terra::app(x = modDat_1_rast, fun = function(x) {
    ceiling(x/100)
  }) %>% 
  subst(from = c(0), to = c(1)) %>% 
  subst(from = c(NA), to = c(100)) %>% 
  subst(from = c(1), to = c(200)) 
  

testBadLayer <- sum(c(notForest_TEST, modDat_1_rast_TEST))
# get only the locations w/ 200, which are places where we have data for modDat but *not* for notForest (bad)
badLayer <- testBadLayer %>% 
  subst(from = c(100, 101, 201), c(NA))
plot(testBadLayer)
plot(badLayer, col = "red", add = TRUE)
```
...
ROC Curves based on classification accuracy of the model when used on the testing set (held-out years 2003, 2012 and 2019)
...
# add inset
           inset_plot <-  ggplot(pred_glm1_full) + 
             geom_density(aes(TotalTreeCover_binom_pred)) +
                theme_bw(base_size=9) +  ## makes everything smaller
                theme(panel.background = element_rect(fill="transparent"),  ## white plot background
                      panel.border = element_blank(),
          axis.title.y = element_blank(),
