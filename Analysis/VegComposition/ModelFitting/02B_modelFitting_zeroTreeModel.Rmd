...
We will fit a binomial model that predicts whether or not there are trees at a location. 
```{r}
...
ggplot(modDat_1_s) + 
  ...
  xlab("P(Non-forest)") + 
  ggtitle("P(Non-Forest) in the training data")
...
pred_glm1_full <- predict_by_response(fit_glm_bestLambda, modDat_1_s)#X[,2:ncol(X)])
...
## for full dataset
pred_glm1_1se_full <- predict_by_response(mod_secondBest, modDat_1_s)#X[,2:ncol(X)])
...
DATA$ID <- 1:nrow(DATA)
#pred.prev <- predicted.prevalence(DATA = DATA[,c("ID", "obs", "pred_opt")], threshold = 20)
...
roc_curve <- roc(pred_glm1_full[,response], pred_glm1_full[,paste0(response, "_pred")])
# Plot ROC curve
...
# Now make an ROC curve based on the test set
roc_curve_secondBest <- roc(pred_glm1_1se_full[,response], pred_glm1_1se_full[,paste0(response, "_pred")])
# Plot ROC curve
...
We use the thresholds identified above to make continuous predictions of P(no trees) binary
```{r}
## for full dataset
# best lambda model
...
pred_glm1_full[pred_glm1_full$fracNotForest == 1 & 
                 pred_glm1_full$fracNotForest_pred_binary == 1, "missClass"] <- 1
pred_glm1_full[pred_glm1_full$fracNotForest == 0 & 
...
pred_glm1_1se_full[pred_glm1_1se_full$fracNotForest == 1 & 
                 pred_glm1_1se_full$fracNotForest_pred_binary == 1, "missClass"] <- 1
pred_glm1_1se_full[pred_glm1_1se_full$fracNotForest == 0 & 
...
Identify Misclassifications, only in non-forest locations
```{r}
# read in .tif w/ predictions of forest classification on dayMet grid
...
```{r Residual maps, fig.height=9, fig.width=9}
...
pred_glm1_full_sf <- pred_glm1_full %>% 
  ...
  terra::vect()
testRast2 <- lapply(c("fracNotForest", "fracNotForest_pred_binary", "resid", "missClass"), FUN = function(x) {
  # S4 method for class 'data.frame'
  if (x %in% c("fracNotForest", "fracNotForest_pred_binary", "resid")) {
...
pred_glm1_full_RAST <- c(testRast2[[1]], testRast2[[2]], testRast2[[3]], testRast2[[4]])
...
labs(title = paste0("Observations of tree presence"), x = "", y = ""
     ) +
...
#f map_obs_cont <-    map_obs_cont# +   #annotation_custom(grob = ggplotGrob(inset_plot), ymin = -2028000, ymax=-1205000, xmin=-2280000, xmax=-180000 ) 
...
```{r predicted values map best lambda model, fig.height = 9, fig.width = 9 }
...
```{r predicted values map 1se lambda model, fig.height = 10, fig.width = 9, }
if ( name_secondBestMod == "Intercept_Only") {
  print("The next best lambda model only contains one predictor (an intercept)")
} else {
...
# make a multi-layer raster w/ layers for each variable of interest
testRast3 <- lapply(c("fracNotForest", "fracNotForest_pred_binary", "resid", "missClass"), FUN = function(x) {
  # S4 method for class 'data.frame'
 if (x %in% c("fracNotForest", "fracNotForest_pred_binary", "resid")) {
...
pred_glm1_1se_full_RAST <- c(testRast3[[1]], testRast3[[2]], testRast3[[3]], testRast3[[4]])
...
geom_raster(data = pred_glm1_w2w_RAST, aes(x = x, y = y, fill = fracNotForest_pred_binary), maxcell = 20000000) + 
  geom_sf(data=cropped_states_2 ,fill=NA ) +
...
geom_raster(data = pred_glm1_1se_w2w_RAST, aes(x = x, y = y, fill = fracNotForest_pred_binary), maxcell = 20000000) + 
  geom_sf(data=cropped_states_2 ,fill=NA ) +
...
```{r}
response_vars <- c("fracNotForest", "fracNotForest_pred_binary")
...
#     select(Year, x, y, fracNotForest_binary, fracNotForest_binary_pred_binary, name, value) %>% 
#     ggplot() + 
#     facet_wrap(~name, scales = "free") + 
#     geom_point(aes(x = value, y = jitter(fracNotForest_binary)), alpha = .5) + 
#     geom_point(aes(x = value, y = jitter(fracNotForest_binary_pred_binary)), col = "blue", alpha = .5) + 
#     geom_smooth(aes(x = value, y = fracNotForest_binary), col = "black") + 
#     geom_smooth(aes(x = value, y = fracNotForest_binary_pred_binary), col = "blue") 
#                  
  
  
  #####
# publication quality version
g3 <- decile_dotplot_pq(df = pred_glm1_deciles, response = c("fracNotForest", paste0("fracNotForest", "_pred_binary")), IQR = FALSE,
                        CI = FALSE
                        ) + ggtitle("Quantile Plot")

g4 <- add_dotplot_inset(g3, df = pred_glm1_deciles, response = c("fracNotForest", paste0("fracNotForest", "_pred_binary")), dfRaw = pred_glm1_full, add_smooth = TRUE, deciles = FALSE)
...
# publication quality version
g3 <- decile_dotplot_pq(pred_glm1_deciles_1se, response = c("fracNotForest", paste0("fracNotForest", "_pred_binary")), IQR = FALSE) + ggtitle("Quantile Plot")

g4 <- add_dotplot_inset(g3, df = pred_glm1_deciles_1se, response = c("fracNotForest", paste0("fracNotForest", "_pred_binary")), dfRaw = pred_glm1_1se_full, add_smooth = TRUE, deciles = FALSE)
