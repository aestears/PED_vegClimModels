...
```{r}
mod_glmFinal <- readRDS("./models/glm_beta_model_CONUSwide_HerbCover_n32000.RDS")

if (byRegion == TRUE) {
  mod_glmFinal_EF <- readRDS("./models/glm_beta_model_EasternForests_HerbCover_n32000.RDS")
mod_glmFinal_WF <- readRDS("./models/glm_beta_model_WesternForests_HerbCover_n32000.RDS")
mod_glmFinal_G <- readRDS("./models/glm_beta_model_GrassShrub_HerbCover_n32000.RDS")
...
plotResid_rast_ALL <- pred_glm1_ALL %>% 
  ...
  terra::crop(ext(-2000000, 2500000, -2000000, 1200000))
# plot
residMap_allHerbs <- ggplot() + 
  geom_spatraster(data = plotResid_rast_ALL) + 
  ggtitle(paste0("Resids. from regional models of herb cover")) + 
  scale_fill_viridis_c(option = "turbo", limits = c(-.55, .91), na.value = "white", 
                       ) #+ 
...
predsMap_allHerbs <- ggplot() + 
  geom_spatraster(data = plotPreds_rast_ALL*100) + 
  ggtitle(paste0("Predictions from regional models of herb cover")) + 
  scale_fill_viridis_c(option = "viridis", limits = c(0, 100), na.value = "white", 
                       ) #+ 1

predsMap_allHerbs / residMap_allHerbs
}
...
max <- df %>% 
    select(HerbCover_dec, HerbCover_dec_pred) %>% 
    max * 100
inset <- ggplot(df, aes(HerbCover_dec*100, 
                        HerbCover_dec_pred*100)) +
    geom_point(shape = 4, alpha = 0.5, size = 0.3) +
    geom_abline(slope = 1, size = 0.5) +
    labs(y = "Observed Herb Cover (%) - deciles",
         x = "Predicted Herb Cover (%) - deciles") +
...
max <- df %>% 
    select(HerbCover_dec, HerbCover_dec_pred) %>% 
    max * 100
inset2 <- ggplot(df, aes(HerbCover_dec*100, 
                        HerbCover_dec_pred*100)) +
    geom_point(shape = 4, alpha = 0.5, size = 0.3) +
    geom_abline(slope = 1, size = 0.5) +
    labs(y = "Observed Herb Cover (%) - deciles",
