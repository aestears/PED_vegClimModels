---
title: "Comparing model performance for different runs of the same model"
author: "Alice Stears"
...
Now, show the different model performance metrics for the models that either include or exclude anomalies 
```{r}
knitr::kable(format = "html", modMetrics_CONUS_shrub,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "No trim anomalies: CONUS - total shrub Cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

knitr::kable(format = "html", modMetrics_CONUS_shrub_trimAnom,
             col.names = c("best Lambda model", "1/2 se lambda model", "1 se lambda model"),
              caption = "Yes trim anomalies: CONUS - total shrub Cover"
) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
names(forecastClimSoilsDatPred_1)[4:53] <- str_remove(names(forecastClimSoilsDatPred_1)[4:53], pattern = "_s$")
...
names(forecastClimSoilsDatPred_2)[4:53] <- str_remove(names(forecastClimSoilsDatPred_2)[4:53], pattern = "_s$")
...
### Read in the NO trim anomalies model objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod_NOtrimAnoms <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/ShrubCover_CONUS_noTLP_FALSE_bestLambdaGLM.rds")

ModelSpec_bestLambda_NOtrimAnoms <- getModelStatement(coefficientTable = CONUS_shrub,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "ShrubCover")
```
...
```{r}
(ModelSpec_bestLambda_NOtrimAnoms$scaledInputVars_ModelStatement)
```


### Read in the trim anomalies model objects
```{r}
# read in model objects (is the trim anomaly version)
 bestLambdaMod_trimAnoms <- readRDS("/Users/astears/Documents/Dropbox_static/Work/NAU_USGS_postdoc/PED_vegClimModels/Analysis/VegComposition/ModelFitting/models/ShrubCover_CONUS_noTLP_FALSE_trimAnom_bestLambdaGLM.rds")

ModelSpec_bestLambda_trimAnom <- getModelStatement(coefficientTable = CONUS_shrub_trimAnoms,
                                                    modelName <- "coefficientValue_bestLambda", 
                                                    responseVar <- "ShrubCover")
```
...
This is the **Best Lambda** model equation if the inputs *are* scaled: 
```{r}
(ModelSpec_bestLambda_trimAnom$scaledInputVars_ModelStatement)
```


Predict Shrub Cover using best lambda models
```{r predict with NO trim anoms model, fig.width = 14, fig.height = 8, message = FALSE}
### predict for no trim anomaly model
# predict w/ best SE lambda model
bestLambda_predict_NOtrimAnoms <- makePredictions(predictionDF = climDatPred, 
                                                           modelObject = bestLambdaMod_NOtrimAnoms)
# predict with best SE lambda model w/ forecasted climate data (model 1)
bestLambda_predictFutureMod1_NOtrimAnoms <- makePredictions(predictionDF = forecastClimSoilsDatPred_1, 
                                                           modelObject = bestLambdaMod_NOtrimAnoms)
# predict with best SE lambda model w/ forecasted climate data (model 2)
bestLambda_predictFutureMod2_NOtrimAnoms <- makePredictions(predictionDF = forecastClimSoilsDatPred_2, 
                                                           modelObject = bestLambdaMod_NOtrimAnoms)
...
plotObs_NOtrimAnoms <- bestLambda_predict_NOtrimAnoms %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
tempExt <- crds(plotObs_NOtrimAnoms, na.rm = TRUE)

plotObs_NOtrimAnoms_2 <- plotObs_NOtrimAnoms %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )

## map best SE lambda predictions for the future (model 1)
plotObs_NOtrimAnoms_Future1 <- bestLambda_predictFutureMod1_NOtrimAnoms %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObs_NOtrimAnoms_Future1_2 <- plotObs_NOtrimAnoms_Future1 %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )
## map best SE lambda predictions for the future (model 2)
plotObs_NOtrimAnoms_Future2 <- bestLambda_predictFutureMod2_NOtrimAnoms %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObs_NOtrimAnoms_Future2_2 <- plotObs_NOtrimAnoms_Future2 %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )
...
# ggplot() + 
#   geom_point(aes(x, y, col = ShrubCover))
...
# make figures
map_NOtrimAnoms <- ggplot() +
geom_spatraster(data = plotObs_NOtrimAnoms_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_NOtrimAnoms_2)),fill=NA ) +
labs(title = paste0("Predictions of shrub cover, 
CONUS-wide with 
contemporary climate data"),
      subtitle = "Using the bestLambda model *without* trimming anomalies") +
  scale_fill_gradient2(low = "brown",
                       mid = "wheat" ,
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
  xlim(st_bbox(plotObs_NOtrimAnoms_2)[c(1,3)]) + 
  ylim(st_bbox(plotObs_NOtrimAnoms_2)[c(2,4)])

map_NOtrimAnoms_future1 <- ggplot() +
geom_spatraster(data = plotObs_NOtrimAnoms_Future1_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_NOtrimAnoms_Future1_2)),fill=NA ) +
labs(title = paste0("Predictions of shrub cover, 
CONUS-wide with forecasted 
climate data - BNU-ESM model"),
      subtitle = "Using the bestLambda model *without* trimming anomalies") +
  scale_fill_gradient2(low = "brown",
                       mid = "wheat" ,
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
  xlim(st_bbox(plotObs_NOtrimAnoms_Future1_2)[c(1,3)]) + 
  ylim(st_bbox(plotObs_NOtrimAnoms_Future1_2)[c(2,4)])

map_NOtrimAnoms_future2 <- ggplot() +
geom_spatraster(data = plotObs_NOtrimAnoms_Future2_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_NOtrimAnoms_Future2_2)),fill=NA ) +
labs(title = paste0("Predictions of shrub cover, 
CONUS-wide with forecasted 
climate data - IPSL-CM5A-MR model"),
      subtitle = "Using the bestLambda model *without* trimming anomalies") +
  scale_fill_gradient2(low = "brown",
                       mid = "wheat" ,
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
  xlim(st_bbox(plotObs_NOtrimAnoms_Future2_2)[c(1,3)]) + 
  ylim(st_bbox(plotObs_NOtrimAnoms_Future2_2)[c(2,4)])

map_obs <- ggplot() +
  geom_spatraster(data = plotObservations_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObservations_2)),fill=NA ) +
labs(title = paste0("Observations of ShrubCover in the 
  ...
  scale_fill_gradient2(low = "brown",
                       ...
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
  xlim(st_bbox(plotObservations_2)[c(1,3)]) + 
  ylim(st_bbox(plotObservations_2)[c(2,4)])

hist_NOtrimAnoms <- ggplot(bestLambda_predict_NOtrimAnoms) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") +
  xlab("Predicted Value") + 
  ylab("frequency")+ 
  xlim(c(0,100))

hist_NOtrimAnoms_future1 <-  ggplot(bestLambda_predictFutureMod1_NOtrimAnoms) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency")+ 
  xlim(c(0,100))

hist_NOtrimAnoms_future2 <-  ggplot(bestLambda_predictFutureMod2_NOtrimAnoms) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency")+ 
  xlim(c(0,100))
...
```{r predict with YES trim anoms model, fig.width = 14, fig.height = 8, message = FALSE}
### predict for yes trim anomaly model
# predict w/ best SE lambda model
bestLambda_predict_trimAnoms <- makePredictions(predictionDF = climDatPred, 
                                                           modelObject = bestLambdaMod_trimAnoms)
# predict with best SE lambda model w/ forecasted climate data (model 1)
bestLambda_predictFutureMod1_trimAnoms <- makePredictions(predictionDF = forecastClimSoilsDatPred_1, 
                                                           modelObject = bestLambdaMod_trimAnoms)
# predict with best SE lambda model w/ forecasted climate data (model 2)
bestLambda_predictFutureMod2_trimAnoms <- makePredictions(predictionDF = forecastClimSoilsDatPred_2, 
                                                           modelObject = bestLambdaMod_trimAnoms)


# predict w/ best model
plotObs_trimAnoms <- bestLambda_predict_trimAnoms %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
# get the extent of this particular raster, and crop it accordingly
tempExt <- crds(plotObs_trimAnoms, na.rm = TRUE)

plotObs_trimAnoms_2 <- plotObs_trimAnoms %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )

## map best SE lambda predictions for the future (model 1)
plotObs_trimAnoms_Future1 <- bestLambda_predictFutureMod1_trimAnoms %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObs_trimAnoms_Future1_2 <- plotObs_trimAnoms_Future1 %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )
## map best SE lambda predictions for the future (model 2)
plotObs_trimAnoms_Future2 <- bestLambda_predictFutureMod2_trimAnoms %>% 
         #drop_na(paste(response)) %>% 
  #slice_sample(n = 5e4) %>%
  terra::vect(geom = c("x", "y")) %>% 
  terra::set.crs(crs(test_rast)) %>% 
...
plotObs_trimAnoms_Future2_2 <- plotObs_trimAnoms_Future2 %>% 
  crop(ext(min(tempExt[,1]), max(tempExt[,1]),
           min(tempExt[,2]), max(tempExt[,2])) 
       )

# make figures
map_trimAnoms <- ggplot() +
geom_spatraster(data = plotObs_trimAnoms_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_trimAnoms_2)),fill=NA ) +
labs(title = paste0("Predictions of shrub cover,
CONUS-wide with contemporary 
climate data"),
      subtitle = "Using the bestLambda model *with* trimming anomalies") +
  scale_fill_gradient2(low = "brown",
                       mid = "wheat" ,
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
  xlim(st_bbox(plotObs_trimAnoms_2)[c(1,3)]) + 
  ylim(st_bbox(plotObs_trimAnoms_2)[c(2,4)])

map_trimAnoms_future1 <- ggplot() +
geom_spatraster(data = plotObs_trimAnoms_Future1_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_trimAnoms_Future1_2)),fill=NA ) +
labs(title = paste0("Predictions of shrub cover, 
CONUS-wide with forecasted 
climate data - BNU-ESM model"),
      subtitle = "Using the bestLambda model *with* trimming anomalies") +
  scale_fill_gradient2(low = "brown",
                       ...
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
  xlim(st_bbox(plotObs_trimAnoms_Future1_2)[c(1,3)]) + 
  ylim(st_bbox(plotObs_trimAnoms_Future1_2)[c(2,4)])

map_trimAnoms_future2 <- ggplot() +
geom_spatraster(data = plotObs_trimAnoms_Future2_2) + 
  geom_sf(data = mapRegions, fill = NA, col = "rosybrown4", lwd = .5) +
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(plotObs_trimAnoms_Future2_2)),fill=NA ) +
labs(title = paste0("Predictions of shrub cover, 
CONUS-wide with forecasted 
climate data - IPSL-CM5A-MR model"),
      subtitle = "Using the bestLambda model *with* trimming anomalies") +
  scale_fill_gradient2(low = "brown",
                       ...
                       high = "darkgreen" , 
                       midpoint = 0, limits = c(0,100),  na.value = "lightgrey") + 
  xlim(st_bbox(plotObs_trimAnoms_Future2_2)[c(1,3)]) + 
  ylim(st_bbox(plotObs_trimAnoms_Future2_2)[c(2,4)])

hist_trimAnoms <- ggplot(bestLambda_predict_trimAnoms) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") +
  xlab("Predicted Value") + 
  ylab("frequency") + 
  xlim(c(0,100))

hist_trimAnoms_future1 <-  ggplot(bestLambda_predictFutureMod1_trimAnoms) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency")+ 
  xlim(c(0,100))

hist_trimAnoms_future2 <-  ggplot(bestLambda_predictFutureMod2_trimAnoms) + 
  geom_density(aes(modelPreds), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency")+ 
  xlim(c(0,100))
...
```{r get residuals for NO trim anomalies model, fig.width = 14, fig.height = 8, message = FALSE, warning=FALSE}
# calculate residuals (observed - predicted)
resids_NOtrimAnoms <-  plotObservations_2 - plotObs_NOtrimAnoms_2 
map_resids_NOtrimAnoms <- ggplot() +
geom_spatraster(data = resids_NOtrimAnoms) + 
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(resids_NOtrimAnoms)),fill=NA )  + 
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Resids. (obs. - pred.) from the CONUS-wide model of ShrubCover"),
     subtitle = "using predictions from the *NoTrimAnomalies* 
  ...
  xlim(st_bbox(resids_NOtrimAnoms)[c(1,3)]) + 
  ylim(st_bbox(resids_NOtrimAnoms)[c(2,4)])

hist_NOtrimAnoms_resids <-  ggplot(resids_NOtrimAnoms) + 
  geom_density(aes(mean), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency") + 
  geom_vline(aes(xintercept = mean(terra::values(resids_NOtrimAnoms$mean), na.rm = TRUE)))

```

```{r get residuals for YES trim anomalies model, fig.width = 14, fig.height = 8, message = FALSE, warning=FALSE}
# calculate residuals (observed - predicted)
resids_trimAnoms <-  plotObservations_2 - plotObs_trimAnoms_2 
map_resids_trimAnoms <- ggplot() +
geom_spatraster(data = resids_trimAnoms) + 
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(resids_trimAnoms)),fill=NA )  + 
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Resids. (obs. - pred.) from the CONUS-wide model of ShrubCover"),
     subtitle = "using predictions from the *NoTrimAnomalies* 
     bestLambda model with contemporary climate data") +
  ...
  xlim(st_bbox(resids_trimAnoms)[c(1,3)]) + 
  ylim(st_bbox(resids_trimAnoms)[c(2,4)])

hist_trimAnoms_resids <-  ggplot(resids_trimAnoms) + 
  geom_density(aes(mean), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency") + 
  geom_vline(aes(xintercept = mean(terra::values(resids_trimAnoms$mean), na.rm = TRUE)))
```

```{r compare forecast from climate models to contemporary predictions NO trim anomalies, fig.width = 14, fig.height = 8, message = FALSE, warning=FALSE}
## from model 1
# calculate deltas (future model predictions - contemporary model predictions)
predDeltas_model1_NOtrimAnoms <-  plotObs_NOtrimAnoms_Future1_2 - plotObs_NOtrimAnoms_2 

map_deltas_model1_NOtrimAnoms <- ggplot() +
geom_spatraster(data = predDeltas_model1_NOtrimAnoms) + 
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(resids_NOtrimAnoms)),fill=NA )  + 
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Future Climate Model Deltas for \n CONUS-wide model of ShrubCover; \n (models with predictions with modeled climate data from model #1 - \n models with predictions from contemporary climate data)"),
     subtitle = "using predictions from the *NoTrimAnomalies* 
     bestLambda models") +
...
hist_deltas_model1_NOtrimAnoms <-  ggplot(predDeltas_model1_NOtrimAnoms) + 
  geom_density(aes(mean), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency") 
...
## from model 1
# calculate deltas (future model predictions - contemporary model predictions)
predDeltas_model2_NOtrimAnoms <-  plotObs_NOtrimAnoms_Future2_2 - plotObs_NOtrimAnoms_2 

map_deltas_model2_NOtrimAnoms <- ggplot() +
geom_spatraster(data = predDeltas_model2_NOtrimAnoms) + 
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(resids_NOtrimAnoms)),fill=NA )  + 
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Future Climate Model Deltas for \n CONUS-wide model of ShrubCover; \n (models with predictions with modeled climate data from model #2 - \n models with predictions from contemporary climate data)"),
     subtitle = "using predictions from the *NoTrimAnomalies* 
     bestLambda models") +
  scale_fill_gradient2(low = "orange",
                       mid = "white" ,
...
hist_deltas_model2_NOtrimAnoms <-  ggplot(predDeltas_model2_NOtrimAnoms) + 
  geom_density(aes(mean), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency") 
```

```{r compare forecast from climate models to contemporary predictions YES trim anomalies, fig.width = 14, fig.height = 8, message = FALSE, warning=FALSE}
## from model 1
# calculate deltas (future model predictions - contemporary model predictions)
predDeltas_model1_trimAnoms <-  plotObs_trimAnoms_Future1_2 - plotObs_trimAnoms_2 

map_deltas_model1_trimAnoms <- ggplot() +
geom_spatraster(data = predDeltas_model1_trimAnoms) + 
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(resids_trimAnoms)),fill=NA )  + 
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Future Climate Model Deltas for \n CONUS-wide model of ShrubCover; \n (models with predictions with modeled climate data from model #1 - \n models with predictions from contemporary climate data)"),
     subtitle = "using predictions from the *TrimAnomalies* 
     bestLambda models") +
  scale_fill_gradient2(low = "orange",
                       mid = "white" ,
                       high = "purple" , 
...
hist_deltas_model1_trimAnoms <-  ggplot(predDeltas_model1_trimAnoms) + 
  geom_density(aes(mean), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency") 
...
## from model 2
# calculate deltas (future model predictions - contemporary model predictions)
predDeltas_model2_trimAnoms <-  plotObs_trimAnoms_Future2_2 - plotObs_trimAnoms_2 

map_deltas_model2_trimAnoms <- ggplot() +
geom_spatraster(data = predDeltas_model2_trimAnoms) + 
  geom_sf(data=cropped_states %>% st_transform(crs = st_crs(test_rast)) %>% st_crop(st_bbox(resids_trimAnoms)),fill=NA )  + 
  geom_sf(data = mapRegions, fill = NA, col = "orchid", lwd = .5) +
  labs(title = paste0("Future Climate Model Deltas for \n CONUS-wide model of ShrubCover; \n (models with predictions with modeled climate data from model #2 - \n models with predictions from contemporary climate data)"),
     subtitle = "using predictions from the *TrimAnomalies* 
...
hist_deltas_model2_trimAnoms <-  ggplot(predDeltas_model2_trimAnoms) + 
  geom_density(aes(mean), fill = "lightgrey", col = "darkgrey") + 
  xlab("Predicted Value") + 
  ylab("frequency") 
```

### Here are the observations of shrub cover across CONUS
```{r make map of observations, fig.width = 8, fig.height = 8, message = FALSE}
## make maps 
library(ggpubr)
# observations
...
### Here are the predictions of shrub cover using the no trim anomalies model
```{r make maps for no trim anoms, fig.width = 16, fig.height = 18, message = FALSE}
  # plot model forecasts with model that does not exclude anomalies
  ggarrange(map_NOtrimAnoms, map_NOtrimAnoms_future1, map_NOtrimAnoms_future2,
            hist_NOtrimAnoms, hist_NOtrimAnoms_future1, hist_NOtrimAnoms_future2, 
            map_resids_NOtrimAnoms, map_deltas_model1_NOtrimAnoms, map_deltas_model2_NOtrimAnoms,
            hist_NOtrimAnoms_resids, hist_deltas_model1_NOtrimAnoms, hist_deltas_model2_NOtrimAnoms,
            heights = c(3,1), ncol = 3, nrow = 4) %>% 
  annotate_figure(fig.lab = "Model Predictions with Contemporary and Projected Climate Data - *without* trimming anomalies", fig.lab.size = 20)
```

```{r make maps for trim anoms, fig.width = 16, fig.height = 18, message = FALSE}

  # plot model forecasts with model that does not exclude anomalies
     ggarrange(map_trimAnoms, map_trimAnoms_future1, map_trimAnoms_future2,
            hist_trimAnoms, hist_trimAnoms_future1, hist_trimAnoms_future2, 
            map_resids_trimAnoms, map_deltas_model1_trimAnoms, map_deltas_model2_trimAnoms,
            hist_trimAnoms_resids, hist_deltas_model1_trimAnoms, hist_deltas_model2_trimAnoms,
            heights = c(3,1), ncol = 3, nrow = 4) %>% 
  annotate_figure(fig.lab = "Model Predictions with Contemporary and Projected Climate Data - *with* trimming anomalies", fig.lab.size = 20)
```
