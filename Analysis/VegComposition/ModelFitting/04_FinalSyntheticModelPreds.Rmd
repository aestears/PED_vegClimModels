...
totalTree_F <- # include anomalies option
  #readRDS("./models/betaLASSO/TotalTreeCover_forest_noTLP_FALSE_removeAnomaliesFALSE_trimAnom_bestLambdaGLM.rds")
  #remove anomalies option #
...
Once we generate predictions using each of these models, we transform the ecoregion-level predictions into CONUS-wide predictions by scaling them according to predicted ecoregion probability from the ecoregion classification model. 
...
Then, for those models that are predicting proportions, we scale all of the relevant predictions so they sum to one. 
...
Finally, we convert these predicted proportions into predictions of absolute cover. 
(e.g.  absolute cover of needle-leaved trees  = scaled proportion of total herbaceous cover that is forbs * absolute cover of total trees)
```{r}
## for contemporary data
contempPreds <- contempPreds %>% 
...
deciles_totalTree_GS_entire <- predvars2deciles(df = preds_quantile %>% select(TotalTreeCover, absTotalTree_GS,
...
quantPlot_totalTree_GS_entire <- add_dotplot_inset(quantPlot_totalTree_GS_entire, df = deciles_totalTree_GS_entire, response = c("TotalTreeCover", "absTotalTree_GS"), dfRaw = preds_quantile %>% select(TotalTreeCover, absTotalTree_GS,  prcp, prcp_seasonality, sand ,AWHC) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)


# for only GS (EPA defined) using the GS model

deciles_totalTree_GS_GS <- predvars2deciles(df = preds_quantile %>% filter(newRegion == "dryShrubGrass") %>% select(TotalTreeCover, absTotalTree_GS,
                                                                               prcp, prcp_seasonality, sand ,AWHC) %>% drop_na()
                                   , response_vars = c("TotalTreeCover", "absTotalTree_GS"), # name of observations, followed by name of predictions
                                   pred_vars = c("prcp", "prcp_seasonality","sand" ,"AWHC"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc NL tree for GS and F)
cut_points = seq(0, 1, 0.01)
...
quantPlot_totalTree_GS_GS <- add_dotplot_inset(quantPlot_totalTree_GS_GS, df = deciles_totalTree_GS_GS, response = c("TotalTreeCover", "absTotalTree_GS"), dfRaw = preds_quantile %>% filter(newRegion == "dryShrubGrass") %>% select(TotalTreeCover, absTotalTree_GS,
                                                                               prcp, prcp_seasonality, sand ,AWHC)  %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only GS (model-defined using the GS model
deciles_totalTree_GS_GSprob <- predvars2deciles(df = preds_quantile %>% filter(prob_Forest<0.5) %>% select(TotalTreeCover, absTotalTree_GS,
                                                                               prcp, prcp_seasonality, sand ,AWHC) %>% drop_na()
                                   , response_vars = c("TotalTreeCover", "absTotalTree_GS"), # name of observations, followed by name of predictions
                                   pred_vars = c("prcp", "prcp_seasonality","sand" ,"AWHC"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc NL tree for GS and F)
cut_points = seq(0, 1, 0.01)
...
quantPlot_totalTree_GS_GSprob <- add_dotplot_inset(quantPlot_totalTree_GS_GSprob, df = deciles_totalTree_GS_GSprob, response = c("TotalTreeCover", "absTotalTree_GS"), dfRaw = preds_quantile %>% filter(prob_Forest<0.5) %>% select(TotalTreeCover, absTotalTree_GS,
                                                                               prcp, prcp_seasonality, sand ,AWHC)  %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for the entire CONUS using the F model
deciles_totalTree_F_entire <- predvars2deciles(df = preds_quantile %>% select(TotalTreeCover, absTotalTree_F,
...
quantPlot_totalTree_F_entire <- add_dotplot_inset(quantPlot_totalTree_F_entire, df = deciles_totalTree_F_entire, response = c("TotalTreeCover", "absTotalTree_F"), dfRaw = preds_quantile %>% select(TotalTreeCover, absTotalTree_F,  tmean, prcp, prcp_dry, isothermality, AWHC, prcpTempCorr, clay, carbon, coarse, sand) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only F (EPA defined) using the F model
deciles_totalTree_F_F <- predvars2deciles(df = preds_quantile %>% filter(newRegion != "dryShrubGrass") %>% select(TotalTreeCover, absTotalTree_F,tmean, prcp, prcp_dry, isothermality, AWHC, prcpTempCorr, clay, carbon, coarse, sand) %>% drop_na()
                                   , response_vars = c("TotalTreeCover", "absTotalTree_F"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp", "prcp_dry", "isothermality", "AWHC", "prcpTempCorr", "clay", "carbon", "coarse", "sand"), # for predictors, combine list of predictors for all models (total tree for F and F, and perc NL tree for F and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_totalTree_F_F <- add_dotplot_inset(quantPlot_totalTree_F_F, df = deciles_totalTree_F_F, response = c("TotalTreeCover", "absTotalTree_F"), dfRaw = preds_quantile %>% filter(newRegion != "dryShrubGrass") %>% select(TotalTreeCover, absTotalTree_F,
...
deciles_totalTree_F_Fprob <- predvars2deciles(df = preds_quantile %>% filter(prob_Forest>=0.5) %>% select(TotalTreeCover, absTotalTree_F,
                                                                               tmean, prcp, prcp_dry, isothermality, AWHC, prcpTempCorr, clay, carbon, coarse, sand) %>% drop_na()
                                   , response_vars = c("TotalTreeCover", "absTotalTree_F"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp", "prcp_dry", "isothermality", "AWHC", "prcpTempCorr", "clay", "carbon", "coarse", "sand"), # for predictors, combine list of predictors for all models (total tree for F and F, and perc NL tree for F and F)
cut_points = seq(0, 1, 0.01)
...
quantPlot_totalTree_F_Fprob <- add_dotplot_inset(quantPlot_totalTree_F_Fprob, df = deciles_totalTree_F_Fprob, response = c("TotalTreeCover", "absTotalTree_F"), dfRaw = preds_quantile %>% filter(prob_Forest>=0.5) %>% select(TotalTreeCover, absTotalTree_F,
                                                                               prcp, prcp_seasonality, sand ,AWHC)  %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

#### NL trees ####
# for the entire CONUS using the GS model
deciles_NLtree_RawGS_entire <- predvars2deciles(df = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_GS, tmean, prcp, AWHC, prcpTempCorr, isothermality, prcpTempCorr_anom, coarse) %>% drop_na()
...
quantPlot_NLtree_RawGS_entire <- add_dotplot_inset(quantPlot_NLtree_RawGS_entire, df = deciles_NLtree_RawGS_entire, response = c("ConifTreeCover_prop", "percNLTree_GS"), dfRaw = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_GS, tmean, prcp, AWHC, prcpTempCorr, isothermality, prcpTempCorr_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)


# for only GS using the GS model (EPA-defined)
deciles_NLtree_RawGS_GS <- predvars2deciles(df = preds_quantile %>% filter(newRegion == "dryShrubGrass") %>% select(ConifTreeCover_prop, percNLTree_GS, tmean, prcp, AWHC, prcpTempCorr, isothermality, prcpTempCorr_anom, coarse) %>% drop_na() 
                                   , response_vars = c("ConifTreeCover_prop", "percNLTree_GS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp", "AWHC", "prcpTempCorr", "isothermality", "prcpTempCorr_anom", "coarse"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc NL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_NLtree_RawGS_GS <- add_dotplot_inset(quantPlot_NLtree_RawGS_GS, df = deciles_NLtree_RawGS_GS, response = c("ConifTreeCover_prop", "percNLTree_GS"), dfRaw = preds_quantile %>% filter(newRegion == "dryShrubGrass") %>% select(ConifTreeCover_prop, percNLTree_GS, tmean, prcp, AWHC, prcpTempCorr, isothermality, prcpTempCorr_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only GS using the GS model (model-defined)
deciles_NLtree_RawGS_GSprob <- predvars2deciles(df = preds_quantile %>% filter(prob_Forest < 0.5) %>% select(ConifTreeCover_prop, percNLTree_GS, tmean, prcp, AWHC, prcpTempCorr, isothermality, prcpTempCorr_anom, coarse) %>% drop_na() 
                                   , response_vars = c("ConifTreeCover_prop", "percNLTree_GS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp", "AWHC", "prcpTempCorr", "isothermality", "prcpTempCorr_anom", "coarse"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc NL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_NLtree_RawGS_GSprob <- add_dotplot_inset(quantPlot_NLtree_RawGS_GSprob, df = deciles_NLtree_RawGS_GSprob, response = c("ConifTreeCover_prop", "percNLTree_GS"), dfRaw = preds_quantile %>% filter(prob_Forest < 0.5) %>% select(ConifTreeCover_prop, percNLTree_GS, tmean, prcp, AWHC, prcpTempCorr, isothermality, prcpTempCorr_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)


# for the entire CONUS using the F model
deciles_NLtree_RawF_entire <- predvars2deciles(df = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_F, tmean, prcp_dry, sand, carbon, AWHC, isothermality, prcpTempCorr, prcpTempCorr_anom, isothermality_anom, coarse) %>% drop_na()
...
quantPlot_NLtree_RawF_entire <- add_dotplot_inset(quantPlot_NLtree_RawF_entire, df = deciles_NLtree_RawF_entire, response = c("ConifTreeCover_prop", "percNLTree_F"), dfRaw = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_F, tmean, prcp_dry, sand, carbon, AWHC, isothermality, prcpTempCorr, prcpTempCorr_anom, isothermality_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only F (EPA defined) using the F model
deciles_NLtree_RawF_F <- predvars2deciles(df = preds_quantile %>% filter(newRegion != "dryShrubGrass") %>% select(ConifTreeCover_prop, percNLTree_F, tmean, prcp_dry, sand, carbon, AWHC, isothermality, prcpTempCorr, prcpTempCorr_anom, isothermality_anom, coarse) %>% drop_na() 
                                   , response_vars = c("ConifTreeCover_prop", "percNLTree_F"), # name of observations, followed by name of predictions
...
quantPlot_NLtree_RawF_F <- add_dotplot_inset(quantPlot_NLtree_RawF_F, df = deciles_NLtree_RawF_F, response = c("ConifTreeCover_prop", "percNLTree_F"), dfRaw = preds_quantile %>% filter(newRegion != "dryShrubGrass")%>% select(ConifTreeCover_prop, percNLTree_F, tmean, prcp_dry, sand, carbon, AWHC, isothermality, prcpTempCorr, prcpTempCorr_anom, isothermality_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only F (Model-defined) using the F model
deciles_NLtree_RawF_Fprob <- predvars2deciles(df = preds_quantile %>% filter(prob_Forest > 0.5) %>% select(ConifTreeCover_prop, percNLTree_F, tmean, prcp_dry, sand, carbon, AWHC, isothermality, prcpTempCorr, prcpTempCorr_anom, isothermality_anom, coarse) %>% drop_na() 
                                   , response_vars = c("ConifTreeCover_prop", "percNLTree_F"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp_dry", "sand", "carbon", "AWHC", "isothermality", "prcpTempCorr", "prcpTempCorr_anom", "isothermality_anom", "coarse"), # for predictors, combine list of predictors for all models (total tree for F and F, and perc NL tree for F and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_NLtree_RawF_Fprob <- add_dotplot_inset(quantPlot_NLtree_RawF_Fprob, df = deciles_NLtree_RawF_Fprob, response = c("ConifTreeCover_prop", "percNLTree_F"), dfRaw = preds_quantile %>% filter(prob_Forest > 0.5) %>% select(ConifTreeCover_prop, percNLTree_F, tmean, prcp_dry, sand, carbon, AWHC, isothermality, prcpTempCorr, prcpTempCorr_anom, isothermality_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

## proportion of trees that are needle leaved -- scaled according to ecoregion probability 
# NL trees for the entire CONUS (scaled according to model-predicted ecoregion probability)
deciles_NLtree_ScaledByEco_entire <- predvars2deciles(df = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_CONUS, tmean,  prcp,  prcp_dry , isothermality, AWHC, prcpTempCorr, clay,  carbon, coarse, sand ,  prcp_seasonality,   prcpTempCorr_anom , isothermality_anom) %>% drop_na()
                                   , response_vars = c("ConifTreeCover_prop", "percNLTree_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean",  "prcp",  "prcp_dry" , "isothermality", "AWHC", "prcpTempCorr", "clay",  "carbon", "coarse", "sand" ,  "prcp_seasonality",   "prcpTempCorr_anom" , "isothermality_anom"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc NL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
quantPlot_NLtree_ScaledByEco_entire <- decile_dotplot_pq(df = deciles_NLtree_ScaledByEco_entire, response= c("ConifTreeCover_prop", "percNLTree_CONUS"), IQR = TRUE,
                        CI = FALSE
                        ) + ggtitle("Proportion of trees that are Needle-Leaved - scaled by ecoregion to CONUS")

quantPlot_NLtree_ScaledByEco_entire <- add_dotplot_inset(quantPlot_NLtree_ScaledByEco_entire, df = deciles_NLtree_ScaledByEco_entire, response = c("ConifTreeCover_prop", "percNLTree_CONUS"), dfRaw = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_CONUS, tmean,  prcp,  prcp_dry , isothermality, AWHC, prcpTempCorr, clay,  carbon, coarse, sand ,  prcp_seasonality,   prcpTempCorr_anom , isothermality_anom) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

## proportion of trees that are needle leaved -- scaled to sum to 1 w/ prop of broad-leaved tree
# NL trees for the entire CONUS 
deciles_NLtree_ScaledFinal_entire <- predvars2deciles(df = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_scaled_CONUS, tmean,  prcp,  prcp_dry , isothermality, AWHC, prcpTempCorr, clay,  carbon, coarse, sand ,  prcp_seasonality,   prcpTempCorr_anom , isothermality_anom) %>% drop_na()
                                   , response_vars = c("ConifTreeCover_prop", "percNLTree_scaled_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean",  "prcp",  "prcp_dry" , "isothermality", "AWHC", "prcpTempCorr", "clay",  "carbon", "coarse", "sand" ,  "prcp_seasonality",   "prcpTempCorr_anom" , "isothermality_anom"), # for predictors, combine list of predictors for all models (total tree for  and F, and perc NL tree for  and F)
cut_points = seq(0, 1, 0.01)
                                     )
quantPlot_NLtree_ScaledFinal_entire <- decile_dotplot_pq(df = deciles_NLtree_ScaledFinal_entire, response= c("ConifTreeCover_prop", "percNLTree_scaled_CONUS"), IQR = TRUE,
                        CI = FALSE
                        ) + ggtitle("Proportion of trees that are Needle-Leaved - scaled so NL and BL tree prop sums to 1")

quantPlot_NLtree_ScaledFinal_entire <- add_dotplot_inset(quantPlot_NLtree_ScaledFinal_entire, df = deciles_NLtree_ScaledFinal_entire, response = c("ConifTreeCover_prop", "percNLTree_scaled_CONUS"), dfRaw = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_scaled_CONUS, tmean,  prcp,  prcp_dry , isothermality, AWHC, prcpTempCorr, clay,  carbon, coarse, sand ,  prcp_seasonality,   prcpTempCorr_anom , isothermality_anom) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

## Absolute cover of NL trees
# NL trees
# for the entire CONUS using the  model
deciles_NLtree_Abs_entire <- predvars2deciles(df = preds_quantile %>% select(NLTreeCover_abs, absNLTree_CONUS, tmean,  prcp,  prcp_dry , isothermality, AWHC, prcpTempCorr, clay,  carbon, coarse, sand ,  prcp_seasonality,   prcpTempCorr_anom , isothermality_anom) %>% drop_na()
                                   , response_vars = c("NLTreeCover_abs", "absNLTree_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean",  "prcp",  "prcp_dry" , "isothermality", "AWHC", "prcpTempCorr", "clay",  "carbon", "coarse", "sand" ,  "prcp_seasonality",   "prcpTempCorr_anom" , "isothermality_anom"), # for predictors, combine list of predictors for all models (total tree for  and F, and perc NL tree for  and F)
cut_points = seq(0, 1, 0.01)
                                     )
quantPlot_NLtree_Abs_entire <- decile_dotplot_pq(df = deciles_NLtree_Abs_entire, response= c("NLTreeCover_abs", "absNLTree_CONUS"), IQR = TRUE,
                        CI = FALSE
                        ) + ggtitle("Absolute Needle-Leaved tree cover")

quantPlot_NLtree_Abs_entire <- add_dotplot_inset(quantPlot_NLtree_Abs_entire, df = deciles_NLtree_Abs_entire, response = c("NLTreeCover_abs", "absNLTree_CONUS"), dfRaw = preds_quantile %>% select(NLTreeCover_abs, absNLTree_CONUS, tmean,  prcp,  prcp_dry , isothermality, AWHC, prcpTempCorr, clay,  carbon, coarse, sand ,  prcp_seasonality,   prcpTempCorr_anom , isothermality_anom) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)
#### BL trees ####
# for the entire CONUS using the GS model
deciles_BLtree_RawGS_entire <- predvars2deciles(df = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_GS, tmean, prcp, prcpTempCorr, sand, coarse, AWHC, prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality, prcpTempCorr_anom, sand) %>% drop_na()
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_GS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp", "prcpTempCorr", "sand", "coarse", "AWHC", "prcp_seasonality_anom", "isothermality_anom", "isothermality", "prcp_seasonality", "prcpTempCorr_anom", "sand"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc BL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_BLtree_RawGS_entire <- add_dotplot_inset(quantPlot_BLtree_RawGS_entire, df = deciles_BLtree_RawGS_entire, response = c("AngioTreeCover_prop", "percBLTree_GS"), dfRaw = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_GS, tmean, prcp, prcpTempCorr, sand, coarse, AWHC, prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality, prcpTempCorr_anom, sand) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)


# for only GS using the GS model (EPA-defined)
deciles_BLtree_RawGS_GS <- predvars2deciles(df = preds_quantile %>% filter(newRegion == "dryShrubGrass") %>% select(AngioTreeCover_prop, percBLTree_GS, tmean, prcp, prcpTempCorr, sand, coarse, AWHC, prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality, prcpTempCorr_anom, sand) %>% drop_na() 
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_GS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp", "prcpTempCorr", "sand", "coarse", "AWHC", "prcp_seasonality_anom", "isothermality_anom", "isothermality", "prcp_seasonality", "prcpTempCorr_anom", "sand"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc BL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_BLtree_RawGS_GS <- add_dotplot_inset(quantPlot_BLtree_RawGS_GS, df = deciles_BLtree_RawGS_GS, response = c("AngioTreeCover_prop", "percBLTree_GS"), dfRaw = preds_quantile %>% filter(newRegion == "dryShrubGrass") %>% select(AngioTreeCover_prop, percBLTree_GS, tmean, prcp, prcpTempCorr, sand, coarse, AWHC, prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality, prcpTempCorr_anom, sand) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only GS using the GS model (model-defined)
deciles_BLtree_RawGS_GSprob <- predvars2deciles(df = preds_quantile %>% filter(prob_Forest < 0.5) %>% select(AngioTreeCover_prop, percBLTree_GS, tmean, prcp, prcpTempCorr, sand, coarse, AWHC, prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality, prcpTempCorr_anom, sand) %>% drop_na() 
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_GS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp", "prcpTempCorr", "sand", "coarse", "AWHC", "prcp_seasonality_anom", "isothermality_anom", "isothermality", "prcp_seasonality", "prcpTempCorr_anom", "sand"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc BL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_BLtree_RawGS_GSprob <- add_dotplot_inset(quantPlot_BLtree_RawGS_GSprob, df = deciles_BLtree_RawGS_GSprob, response = c("AngioTreeCover_prop", "percBLTree_GS"), dfRaw = preds_quantile %>% filter(prob_Forest < 0.5) %>% select(AngioTreeCover_prop, percBLTree_GS, tmean, prcp, prcpTempCorr, sand, coarse, AWHC, prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality, prcpTempCorr_anom, sand) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)


# for the entire CONUS using the F model
deciles_BLtree_RawF_entire <- predvars2deciles(df = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_F, tmean, prcp_dry, prcpTempCorr, sand, carbon, AWHC, prcpTempCorr_anom, isothermality, prcp, isothermality_anom, coarse) %>% drop_na()
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_F"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp_dry", "prcpTempCorr", "sand", "carbon", "AWHC", "prcpTempCorr_anom", "isothermality", "prcp", "isothermality_anom", "coarse"), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc BL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_BLtree_RawF_entire <- add_dotplot_inset(quantPlot_BLtree_RawF_entire, df = deciles_BLtree_RawF_entire, response = c("AngioTreeCover_prop", "percBLTree_F"), dfRaw = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_F, tmean, prcp_dry, prcpTempCorr, sand, carbon, AWHC, prcpTempCorr_anom, isothermality, prcp, isothermality_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only F (EPA defined) using the F model
deciles_BLtree_RawF_F <- predvars2deciles(df = preds_quantile %>% filter(newRegion != "dryShrubGrass") %>% select(AngioTreeCover_prop, percBLTree_F, tmean, prcp_dry, prcpTempCorr, sand, carbon, AWHC, prcpTempCorr_anom, isothermality, prcp, isothermality_anom, coarse) %>% drop_na() 
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_F"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp_dry", "prcpTempCorr", "sand", "carbon", "AWHC", "prcpTempCorr_anom", "isothermality", "prcp", "isothermality_anom", "coarse"), # for predictors, combine list of predictors for all models (total tree for F and F, and perc BL tree for F and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_BLtree_RawF_F <- add_dotplot_inset(quantPlot_BLtree_RawF_F, df = deciles_BLtree_RawF_F, response = c("AngioTreeCover_prop", "percBLTree_F"), dfRaw = preds_quantile %>% filter(newRegion != "dryShrubGrass")%>% select(AngioTreeCover_prop, percBLTree_F, tmean, prcp_dry, prcpTempCorr, sand, carbon, AWHC, prcpTempCorr_anom, isothermality, prcp, isothermality_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

# for only F (Model-defined) using the F model
deciles_BLtree_RawF_Fprob <- predvars2deciles(df = preds_quantile %>% filter(prob_Forest >= 0.5) %>% select(AngioTreeCover_prop, percBLTree_F, tmean, prcp_dry, prcpTempCorr, sand, carbon, AWHC, prcpTempCorr_anom, isothermality, prcp, isothermality_anom, coarse) %>% drop_na() 
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_F"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean", "prcp_dry", "prcpTempCorr", "sand", "carbon", "AWHC", "prcpTempCorr_anom", "isothermality", "prcp", "isothermality_anom", "coarse"), # for predictors, combine list of predictors for all models (total tree for F and F, and perc BL tree for F and F)
cut_points = seq(0, 1, 0.01)
                                     )
...
quantPlot_BLtree_RawF_Fprob <- add_dotplot_inset(quantPlot_BLtree_RawF_Fprob, df = deciles_BLtree_RawF_Fprob, response = c("AngioTreeCover_prop", "percBLTree_F"), dfRaw = preds_quantile %>% filter(prob_Forest >= 0.5) %>% select(AngioTreeCover_prop, percBLTree_F, tmean, prcp_dry, prcpTempCorr, sand, carbon, AWHC, prcpTempCorr_anom, isothermality, prcp, isothermality_anom, coarse) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

## proportion of trees that are needle leaved -- scaled according to ecoregion probability 
# BL trees for the entire CONUS (scaled according to model-predicted ecoregion probability)
deciles_BLtree_ScaledByEco_entire <- predvars2deciles(df = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_CONUS, tmean,  prcp,  prcpTempCorr, sand, coarse,  AWHC  , prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality,     
prcpTempCorr_anom , prcp_dry , carbon ) %>% drop_na()
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean",  "prcp",  "prcpTempCorr", "sand", "coarse",  "AWHC"  , "prcp_seasonality_anom", "isothermality_anom", "isothermality", "prcp_seasonality",     
"prcpTempCorr_anom" , "prcp_dry" , "carbon" ), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc BL tree for GS and F)
cut_points = seq(0, 1, 0.01)
                                     )
quantPlot_BLtree_ScaledByEco_entire <- decile_dotplot_pq(df = deciles_BLtree_ScaledByEco_entire, response= c("AngioTreeCover_prop", "percBLTree_CONUS"), IQR = TRUE,
                        CI = FALSE
                        ) + ggtitle("Proportion of trees that are Broad-leaved - scaled by ecoregion to CONUS")

quantPlot_BLtree_ScaledByEco_entire <- add_dotplot_inset(quantPlot_BLtree_ScaledByEco_entire, df = deciles_BLtree_ScaledByEco_entire, response = c("AngioTreeCover_prop", "percBLTree_CONUS"), dfRaw = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_CONUS, tmean,  tmean,  prcp,  prcpTempCorr, sand, coarse,  AWHC  , prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality,     
prcpTempCorr_anom , prcp_dry , carbon ) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

## proportion of trees that are needle leaved -- scaled to sum to 1 w/ prop of broad-leaved tree
# BL trees for the entire CONUS 
deciles_BLtree_ScaledFinal_entire <- predvars2deciles(df = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_scaled_CONUS, tmean,  prcp,  prcpTempCorr, sand, coarse,  AWHC  , prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality,     
prcpTempCorr_anom , prcp_dry , carbon ) %>% drop_na()
                                   , response_vars = c("AngioTreeCover_prop", "percBLTree_scaled_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean",  "prcp",  "prcpTempCorr", "sand", "coarse",  "AWHC"  , "prcp_seasonality_anom", "isothermality_anom", "isothermality", "prcp_seasonality",     
"prcpTempCorr_anom" , "prcp_dry" , "carbon" ), # for predictors, combine list of predictors for all models (total tree for  and F, and perc BL tree for  and F)
cut_points = seq(0, 1, 0.01)
                                     )
quantPlot_BLtree_ScaledFinal_entire <- decile_dotplot_pq(df = deciles_BLtree_ScaledFinal_entire, response= c("AngioTreeCover_prop", "percBLTree_scaled_CONUS"), IQR = TRUE,
                        CI = FALSE
                        ) + ggtitle("Proportion of trees that are Broad-leaved - scaled so BL and BL tree prop sums to 1")

quantPlot_BLtree_ScaledFinal_entire <- add_dotplot_inset(quantPlot_BLtree_ScaledFinal_entire, df = deciles_BLtree_ScaledFinal_entire, response = c("AngioTreeCover_prop", "percBLTree_scaled_CONUS"), dfRaw = preds_quantile %>% select(AngioTreeCover_prop, percBLTree_scaled_CONUS, tmean,  prcp,  prcpTempCorr, sand, coarse,  AWHC  , prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality,     
prcpTempCorr_anom , prcp_dry , carbon ) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

## Absolute cover of BL trees
# BL trees for the entire CONUS 
deciles_BLtree_Abs_entire <- predvars2deciles(df = preds_quantile %>% select(BLTreeCover_abs, absBLTree_CONUS,tmean,  prcp,  prcpTempCorr, sand, coarse,  AWHC  , prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality,     
prcpTempCorr_anom , prcp_dry , carbon ) %>% drop_na()
                                   , response_vars = c("BLTreeCover_abs", "absBLTree_CONUS"), # name of observations, followed by name of predictions
                                   pred_vars = c("tmean",  "prcp",  "prcpTempCorr", "sand", "coarse",  "AWHC"  , "prcp_seasonality_anom", "isothermality_anom", "isothermality", "prcp_seasonality",     
"prcpTempCorr_anom" , "prcp_dry" , "carbon" ), # for predictors, combine list of predictors for all models (total tree for  and F, and perc BL tree for  and F)
...
quantPlot_BLtree_Abs_entire <- decile_dotplot_pq(df = deciles_BLtree_Abs_entire, response= c("BLTreeCover_abs", "absBLTree_CONUS"), IQR = TRUE,
                        CI = FALSE
                        ) + ggtitle("Absolute Broad-leaved tree cover")

quantPlot_BLtree_Abs_entire <- add_dotplot_inset(quantPlot_BLtree_Abs_entire, df = deciles_BLtree_Abs_entire, response = c("BLTreeCover_abs", "absBLTree_CONUS"), dfRaw = preds_quantile %>% 
                                                   select(BLTreeCover_abs, absBLTree_CONUS, tmean,  prcp,  prcpTempCorr, sand, coarse,  AWHC  , prcp_seasonality_anom, isothermality_anom, isothermality, prcp_seasonality,     
prcpTempCorr_anom , prcp_dry , carbon ) %>% drop_na(), add_smooth = TRUE, deciles = FALSE)

#### Compare raw data for absolute needle leaved tree cover ####
# observations
ggplot(preds_quantile) + 
  ...
  labs(title = "Observed needle-leaved tree cover", subtitle = "black = total tree cover \norange = % of tree cover that is needle-leaved \nred = absolute needle-leaved tree cover")
  
ggplot(preds_quantile) + 
  geom_density(aes(absTotalTree_CONUS), col = "black") +# modeled total tree cover
  geom_density(aes(percNLTree_scaled_CONUS), col = "orange") + # % of needle-leaved tree (scaled to CONUS-wide)
  geom_density(aes(absNLTree_CONUS), col = "red") + # observed absolute NL tree cover
  #geom_density(aes(absTotalTree_CONUS * percNLTree_scaled_CONUS), col = "grey") + 
  labs(title = "Modeled needle-leaved tree cover", subtitle = "black = total tree cover \norange = % of tree cover that is needle-leaved (CONUS-wide) \nred = absolute needle-leaved tree cover (CONUS-wide)")
