...
### Show quantiles plots for these final model predictions
#### first, we need to predict using the same climate data we used to fit the models (so we can compare to observed data)
```{r}
...
ggplot(preds_quantile) +
  #geom_density(aes(absTotalTree_CONUS)) +
  geom_density(aes(BLTreeCover_abs), col = "brown") + # observed absolute NL cover
  #geom_density(aes(BLTreeCover_abs), col = "darkgreen") + # observed absolute BL cover
  #geom_density(aes(absNLTree_CONUS + absBLTree_CONUS), col = "grey")
  #geom_density(aes(percNLTree_scaled_CONUS), col = "wheat") +
 # geom_density(aes(percNLTree_CONUS), col = "yellow") +
  #geom_density(aes(percBLTree_scaled_CONUS), col = "lightgreen") +
  #geom_density(aes(percBLTree_CONUS), col = "green") +
  geom_density(aes(AngioTreeCover_prop), linetype = 2)+
  #geom_density(aes(ConifTreeCover_prop), linetype = 2) + # observed proportion of total tree cover that is NL 
  #geom_density(aes(absNLTree_CONUS), col = "red")  # predicted absolute NL tree cover
  geom_density(aes(percBLTree_F), col = "purple") +
 geom_density(aes(percBLTree_GS), col = "pink")
# 
# ggplot(preds_quantile, aes(y = ConifTreeCover_prop * TotalTreeCover, # observed absolute NL tree cover
#                            x = percNLTree_scaled_CONUS *absTotalTree_CONUS # modeled absolute NL tree cover
#                              )) + 
#   geom_point(alpha = .1) + 
...
#### Now, calculate quantiles for each functional group 
...
#####
#  NLtreedeciles_byHAND <- preds_quantile %>% 
...
#   drop_na() %>% 
#   pivot_longer(cols = tmean:isothermality_anom, values_to = "predictorValue", names_to = "predictorName")
# 
# deciles_NLtree_BYHAND <- ggplot(data = NLtreedeciles_byHAND) + 
#   facet_wrap(~predictorName, scales = "free") + 
#   geom_point(aes(x = predictorValue, y = NLTreeCover_abs), alpha = .1) + # observed
#   geom_point(aes(x = predictorValue, y = absNLTree_CONUS), alpha = .1, col = "lightblue") +# modeled
#   geom_smooth(aes(x = predictorValue, y = NLTreeCover_abs), col = "darkgrey") + # observed
#   geom_smooth(aes(x = predictorValue, y = absNLTree_CONUS), col = "darkblue") # modeled
#   
#####
# # predictions for percentage of trees that are needle leaved 
# deciles_NLtreePerc <- predvars2deciles(df = preds_quantile %>% select(ConifTreeCover_prop, percNLTree_scaled_CONUS, tmean,  prcp,  prcp_dry , isothermality, AWHC, prcpTempCorr, clay,  carbon, coarse, sand ,  prcp_seasonality,   prcpTempCorr_anom , isothermality_anom) %>% drop_na()
...
# deciles_BLtreePerc <- predvars2deciles(df =  preds_quantile %>% select("AngioTreeCover_prop", "percBLTree_CONUS",  "tmean" ,"prcp" ,  
# "prcp_dry" , "isothermality"  ,"AWHC"  ,  
# "prcpTempCorr" , "clay", "carbon","coarse", "sand","prcp_seasonality",  
# "prcp_seasonality_anom","isothermality_anom", "prcpTempCorr_anom" ), response_vars = c("AngioTreeCover_prop", "percBLTree_CONUS"), # name of observations, followed by name of predictions
#   pred_vars = c( "tmean" ,"prcp" ,  
# "prcp_dry" , "isothermality"  ,"AWHC"  ,  
# "prcpTempCorr" , "clay", "carbon","coarse", "sand","prcp_seasonality",  
# "prcp_seasonality_anom","isothermality_anom", "prcpTempCorr_anom" ), # for predictors, combine list of predictors for all models (total tree for GS and F, and perc BL tree for GS and F)
...
# now, make the quantile plots themselves (note that predictors are scaled) 
```{r Quantile_plots fig.height= 16, fig.width = 12, warning=FALSE, message=FALSE}
# total trees trees 
# absolute cover
...
# # percentage of total tree 
# quantPlot_NLtreePerc <- decile_dotplot_pq(df = deciles_NLtreePerc, response= c("ConifTreeCover_prop", "percNLTree_scaled_CONUS"), IQR = TRUE,
#                         CI = FALSE
#                         ) + ggtitle("Decile Plot")
# 
# quantPlot_NLtreePerc <- add_dotplot_inset(quantPlot_NLtreePerc, df = deciles_NLtreePerc, response = c("ConifTreeCover_prop", "percNLTree_scaled_CONUS"), dfRaw = preds_quantile, add_smooth = TRUE, deciles = FALSE)
# 
# broad-leaved trees 
# absolute cover
...
# percentage of total tree 
# quantPlot_BLtreePerc <- decile_dotplot_pq(df = deciles_BLtreePerc, response= c("AngioTreeCover_prop", "percBLTree_CONUS"), IQR = TRUE,
#                         CI = FALSE
#                         ) + ggtitle("Quantile Plot")
# 
# quantPlot_BLtreePerc <- add_dotplot_inset(quantPlot_BLtreePerc, df = deciles_BLtreePerc, response = c("AngioTreeCover_prop", "percBLTree_CONUS"), dfRaw = preds_quantile, add_smooth = TRUE, deciles = FALSE)
...
### relativize cover so that cover for all groups *except* trees sums to 1
```{r}
## for contemporary data
contempPreds <- contempPreds %>% 
