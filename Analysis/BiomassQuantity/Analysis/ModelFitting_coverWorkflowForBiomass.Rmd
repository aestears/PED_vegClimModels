---
title: "Models of vegetation biomass based on climate predictors"
author: "Alice Stears"
...
```{r fit global model, echo = FALSE}
 ...
 ## break predictors into climate, weather, and soils 
  ...
  mod_string <- paste("response_transformed ~", paste0(prednames, collapse = "+"))
  ...
  y <- as.matrix(modDat_1_s[,"response_transformed"])
```
...
hist <- ggplot(pred_glm1) + 
  geom_histogram(aes(resid_perc), fill = "lightgrey", col = "darkgrey") + 
  geom_text(aes(x = min(resid_perc)*.9, y = 1500, label = paste0("min = ", round(min(resid_perc),2)))) +
  geom_text(aes(x = max(resid_perc)*.9, y = 1500, label = paste0("max = ", round(max(resid_perc),2)))) + 
  geom_vline(aes(xintercept = mean(resid_perc)))
...
for (i in 1:length(sort(unique(modDat_1_s$NA_L2NAME)))) {
  ...
  mutate(resid = .[["obs"]] - .[["pred_opt"]] ,
    ...
    mutate(resid_perc = (resid/.[["obs"]] )*100,
           resid_percGlobMod = (resid_globMod/.[["obs"]] )*100)
...
hist_i <- ggplot(predictionData) +
  geom_histogram(aes(resid_perc), col = "darkgrey", fill = "lightgrey") +
